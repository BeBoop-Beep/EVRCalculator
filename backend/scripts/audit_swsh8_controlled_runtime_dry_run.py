"""Project 17B.3f Fusion Strike controlled runtime dry-run (no writes).

This script executes the production EVR runner path for swsh8/Fusion Strike while
intercepting persistence writes in-memory.

Safety behavior:
- Refuses to run unless --dry-run is set
- Strict allowlist scope: swsh8 only
- Intercepts persistence operations (zero DB writes)
- Forces bounded simulation pack count override for runtime validation
"""
...
DEFAULT_JSON_PATH = Path("logs/audits/swsh8_controlled_runtime_dry_run.json")
...
class WriteSpyHarness:
    ...
    def spy_parent_run_with_price_snapshots(
        ...
        *,
        ...
        return {
            "config_id": "dry-config-swsh8",
            "config_hash": "dry-hash-swsh8",
            "run_id": "dry-run-swsh8",
    ...
    def spy_simulation_outputs(
        ...
        *,
        ...
        return {
            "run_summary_id": "dry-summary-swsh8",
...
@contextmanager
def _swsh8_no_write_context(*, harness: WriteSpyHarness, pack_count: int, config_cls: type):
    if int(pack_count) <= 0:
    ...
    def _resolve_set_config_override(target_set_identifier: str) -> tuple[Any, str]:
        ...
        raise RuntimeError(
            "Controlled runtime dry-run scope violation: only swsh8/fusionStrike is allowlisted "
...
def _assert_swsh8_runtime_contract(*, config_cls: type) -> Dict[str, Any]:
    ...
    if not bool(getattr(config, "SLOT_SCHEMA_RUNTIME_ENABLED", False)):
        raise AssertionError("SLOT_SCHEMA_RUNTIME_ENABLED must be True for swsh8 dry-run")
...
def run_swsh8_controlled_runtime_dry_run(
    *,
    ...
    with _swsh8_no_write_context(harness=harness, pack_count=int(pack_count), config_cls=config_cls):
        orchestrator = evr_runner.EVRRunOrchestrator()
        run_result = orchestrator.run(
            target_set_identifier=TARGET_SET_ID,
            input_source="db",
            run_metadata={
                "trigger": "audit_swsh8_controlled_runtime_dry_run",
                "dry_run": True,
                "no_write": True,
    ...
    if run_result.get("pack_value_vs_cost_comparison") is None:
        failures.append("pack_value_vs_cost_comparison missing from run result")

    payload: Dict[str, Any] = {
        "meta": {
            "generated_at_utc": datetime.now(timezone.utc).isoformat(),
            "project": "17B.3f",
            "script": "audit_swsh8_controlled_runtime_dry_run.py",
        ...
        "target": {
            ...
            "resolved_from_swsh_alias_map": resolved_canonical,
        ...
        "persistence": {
            ...
            "write_tables_touched": sorted(set_intended_counts.keys()),
        },
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="swsh8 controlled runtime dry-run audit (no writes)")
...
def main() -> int:
    ...
    try:
        payload = run_swsh8_controlled_runtime_dry_run(
