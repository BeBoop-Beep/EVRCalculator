"""EBAY E2.16B -- blinded human review server for the Japanese-focused
LANGUAGE-v1 DEVELOPMENT extension corpus
(backend/artifacts/index_fair_value/ebay_e2_16b_japanese_language_development_queue.csv).

DEVELOPMENT ONLY. No certification, no production authority, no policy
freeze. Does NOT modify frozen LANGUAGE-v1 ({KOREAN, CHINESE}) or frozen
COMBINED-IDENTITY-v3. Reuses the proven append-only-history / stable-cursor
/ Previous / Undo / edit-relabel review architecture from
backend/scripts/ebay_e2_16_language_development_review_server.py, but is
built as an entirely NEW, E2.16B-specific module with its own artifact
paths -- it never touches or reuses the E2.16 session files.

BLINDING CONTRACT (enforced, not just documented): this server reads the
reviewer-facing queue CSV, which the E2.16B corpus builder
(build_ebay_e2_16b_japanese_language_development_cohort.py) already wrote
WITHOUT any Language aspect, normalized-language, sampling-stratum, or
policy-output column. This module additionally asserts at startup that
none of `FORBIDDEN_REVIEWER_COLUMNS` is present in the queue file, and the
rendered HTML page only ever interpolates fields from that same
forbidden-column-free row -- so there is no code path by which getItem's
Language aspect, the JAPANESE_ASPECT_PRIMARY/ENGLISH_ASPECT_CONTROL/
JAPANESE_QUERY_HARD_CASE sampling stratum, LANGUAGE-v1's normalized result,
or any COMBINED policy output could reach the reviewer's screen.

Human truth label schema (per E2.16B spec, identical to E2.16): ENGLISH /
NON_ENGLISH / UNCERTAIN, plus an optional free-text
`human_language_if_known`. The reviewer judges the PHYSICAL LISTING/PHOTO
ONLY -- never provider metadata, never the seller title's language claims.
Multiple listing images (image_urls_json) are surfaced because provider
aspects may describe a different variant/card than the photographed one;
if the physical card language cannot be determined cleanly (e.g. multiple
cards/languages in one listing), the reviewer must choose UNCERTAIN rather
than guess.

This module performs NO labeling itself. It only serves the review UI for
Donny to label separately (a future E2.16C task performs the Japanese-aspect
TP/FP/precision/Wilson analysis against the internal raw artifact that
retains the hidden Language aspect and full localizedAspects).
"""
...
REVIEWER_PROTOCOL = "SINGLE_REVIEWER_BLIND_DEVELOPMENT_E2_16B"
...
FORBIDDEN_REVIEWER_COLUMNS = frozenset({
    ...
    "raw_localized_aspects", "development_stratum", "search_language_term",
})
...
def assert_reviewer_blind(rows: list[dict[str, Any]]) -> None:
    ...
    if present:
        raise FreezeRefused("EBAY_E2_16B_REVIEW_BLOCKED_LANGUAGE_EVIDENCE_LEAKED_TO_REVIEWER", str(sorted(present)))
...
def freeze(reviewer_id: str) -> dict[str, Any]:
    if is_frozen():
        raise FreezeRefused("EBAY_E2_16B_REVIEW_BLOCKED_ALREADY_FROZEN")
    rows = load_queue_rows()
    assert_reviewer_blind(rows)
    if not MANIFEST_PATH.exists():
        raise FreezeRefused("EBAY_E2_16B_REVIEW_BLOCKED_MANIFEST_MISSING")
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    recorded_fp = manifest.get("corpus_fingerprint")
    recomputed_fp = cohort_fingerprint(rows)
    if recorded_fp != recomputed_fp:
        raise FreezeRefused("EBAY_E2_16B_REVIEW_BLOCKED_CORPUS_FINGERPRINT_MISMATCH",
    ...
    if incomplete:
        raise FreezeRefused("EBAY_E2_16B_REVIEW_BLOCKED_INCOMPLETE_LABELS", f"{len(incomplete)} rows incomplete, e.g. {incomplete[:5]}")
...
def page(row: Mapping[str, Any], reviewed: int, total: int, position: int = 1, message: str = "") -> str:
    ...
    return f"""<!doctype html><meta charset="utf-8"><title>E2.16B Japanese Language Development Review</title>
<style>body{{font:16px system-ui;max-width:1000px;margin:30px auto}}
.listingimg{{max-height:360px;max-width:480px;margin:4px}}
...
<h1>E2.16B Japanese language DEVELOPMENT review &mdash; {REVIEWER_PROTOCOL}</h1>
<p><i>DEVELOPMENT CORPUS ONLY. Not certification. Not production authority. Does not modify frozen
LANGUAGE-v1 or COMBINED-IDENTITY-v3. Judge only the listing photo(s) shown below -- never provider
metadata, never the seller title's language claims. If the listing shows multiple cards/languages
such that the target card's physical language cannot be cleanly determined, choose UNCERTAIN.</i></p>
<div id="rowHeader">
...
<section><h2>eBay listing ({len(image_urls)} image(s))</h2>
...
def _run_primary_review(reviewer_id: str, port: int) -> None:
    rows = load_queue_rows()
    if not rows:
        raise SystemExit("Queue is empty -- run build_ebay_e2_16b_japanese_language_development_cohort.py first.")
    assert_reviewer_blind(rows)
    ...
    if not manifest.get("review_session_id"):
        manifest["review_session_id"] = f"e2_16b_dev_session_{uuid.uuid4().hex[:12]}"
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="EBAY E2.16B Japanese language development review server")
