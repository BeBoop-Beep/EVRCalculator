...
from backend.scripts.ebay_final_gold_audit import (
    GOLD_MANIFEST, append_audit_event, audit_summary, build_audit_queue,
    build_audit_undo, freeze_gold, read_audit_history, reconstruct_audit_state,
)
...
def audit_page(row: Mapping[str, Any], reviewed: int, total: int) -> str:
    """Render human evidence and current human label, with no matcher fields."""
    e = html.escape
    options = "".join(f'<option>{e(label)}</option>' for label in LABELS)
    return f"""<!doctype html><meta charset="utf-8"><title>Final Blind Gold Audit</title>
<style>body{{font:16px system-ui;max-width:1100px;margin:30px auto}}img{{max-height:360px;max-width:500px}}button{{margin:4px;padding:8px}}.cols{{display:flex;gap:30px}}</style>
<h1>Final Blind human-gold consistency audit</h1><p>{reviewed}/{total} audited · {total-reviewed} remaining</p>
<p><b>Current human label: {e(str(row['current_human_label']))}</b></p>
<div class="cols"><section><h2>Target card</h2><p><b>{e(str(row['target_card_name']))}</b><br>{e(str(row['target_set_name']))}<br>#{e(str(row['target_card_number']))}<br>Variant: {e(str(row['target_treatment']))}<br>Expected language: English</p></section>
<section><h2>Listing evidence</h2><p><b>{e(str(row['listing_title']))}</b><br>Condition: {e(str(row['condition']))}<br>Buying: {e(str(row['buying_options_json']))}</p><img src="{e(str(row['image_url']))}"><p><a href="{e(str(row['item_url']))}" target="_blank">Inspect listing</a></p></section></div>
...
<script>const id={json.dumps(row['benchmark_row_id'])},original={json.dumps(row['current_human_label'])};function send(action){{let reason=prompt('Optional reason')||'';fetch('/event',{{method:'POST',headers:{{'Content-Type':'application/x-www-form-urlencoded'}},body:new URLSearchParams({{row_id:id,action,original_label:original,replacement_label:replacement.value,reason}})}}).then(()=>location.reload())}}keep.onclick=()=>send('audit_keep');replace.onclick=()=>send('audit_replace');undo.onclick=()=>fetch('/event',{{method:'POST',headers:{{'Content-Type':'application/x-www-form-urlencoded'}},body:new URLSearchParams({{action:'audit_undo'}})}}).then(()=>location.reload())</script>"""
...
def main() -> None:
    ...
    parser.add_argument("--audit-final-gold", action="store_true")
    parser.add_argument("--freeze-final-gold", action="store_true")
    args = parser.parse_args()
    if args.audit_final_gold or args.freeze_final_gold:
        args.partition = "FINAL_BLIND_TEST"
    ...
    if args.audit_final_gold or args.freeze_final_gold:
        ...
        base = reconstruct_effective_state(review_events, args.partition, args.reviewer, row_ids)
        if len(base["labels"]) != len(rows) or base["skipped"]:
            raise SystemExit("Final Blind human review must be complete before audit")
        audit_rows = build_audit_queue(rows, base["labels"], review_events, args.reviewer)
        audit_ids = [str(row["benchmark_row_id"]) for row in audit_rows]
        audit_state = reconstruct_audit_state(read_audit_history(), args.reviewer, audit_ids)
        current_summary = audit_summary(base["labels"], audit_state, len(audit_rows))
        print(json.dumps({"partition": args.partition, "reviewer_id": args.reviewer, **current_summary}, sort_keys=True))
        if args.summary:
            return
        if args.freeze_final_gold:
            print(json.dumps(freeze_gold(rows, base["labels"], audit_state, args.reviewer, HISTORY), sort_keys=True))
            return
        if GOLD_MANIFEST.exists():
            raise SystemExit("Final Blind gold is already frozen")
        lock = threading.Lock()
        cursor = {"index": next((i for i, row in enumerate(audit_rows) if row["benchmark_row_id"] in audit_state["unreviewed"]), None)}

        class AuditHandler(BaseHTTPRequestHandler):
            def do_GET(self) -> None:
                with lock:
                    state = reconstruct_audit_state(read_audit_history(), args.reviewer, audit_ids)
                    index = next((i for i, row in enumerate(audit_rows) if row["benchmark_row_id"] in state["unreviewed"]), None)
                    cursor["index"] = index
                    if index is None:
                        body = b"<!doctype html><h1>Audit complete</h1><p>Stop the server and run with --freeze-final-gold.</p>"
                    else:
                        body = audit_page(audit_rows[index], len(state["decisions"]), len(audit_rows)).encode()
                self.send_response(200); self.send_header("Content-Type", "text/html; charset=utf-8"); self.end_headers(); self.wfile.write(body)

            def do_POST(self) -> None:
                length = int(self.headers.get("Content-Length", "0"))
                data = {key: value[0] for key, value in parse_qs(self.rfile.read(length).decode()).items()}
                with lock:
                    if data.get("action") == "audit_undo":
                        event = build_audit_undo(read_audit_history(), args.reviewer, audit_ids)
                        if event: append_audit_event(event)
                    elif (data.get("action") in {"audit_keep", "audit_replace"}
                          and data.get("row_id") in audit_ids):
                        append_audit_event({"partition": args.partition, "reviewer_id": args.reviewer, **data})
        ...
        ThreadingHTTPServer(("127.0.0.1", args.port), AuditHandler).serve_forever()
        return
    lock = threading.Lock()
