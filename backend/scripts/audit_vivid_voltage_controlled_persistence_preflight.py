"""Project 17E.2 Vivid Voltage controlled persistence preflight (read-only).

Scope is hard-locked to swsh4 only. This script verifies identity resolution and
read-surface readiness without any write operations.

Checks:
- sets table resolution for expected target UUID
- latest-row presence in calculation_runs, simulation_latest_by_target,
  and explore_rip_statistics_latest
- get_explore_page_payload resolution when persisted rows are present
"""
...
DEFAULT_JSON_PATH = Path("logs/audits/vivid_voltage_controlled_persistence_preflight.json")

TARGET_SET_ID = "swsh4"
TARGET_ALLOWLIST = (TARGET_SET_ID,)
EXPECTED_TARGET_UUID_BY_SET_ID = {
    "swsh4": "26fedb88-87d7-487a-9f01-528d603c682e",
}
...
def run_vivid_voltage_controlled_persistence_preflight(
    *,
    ...
    payload: Dict[str, Any] = {
        "meta": {
            ...
            "project": "17E.2",
            "script": "audit_vivid_voltage_controlled_persistence_preflight.py",
        ...
        "target": {
            ...
            "expected_uuid": dict(EXPECTED_TARGET_UUID_BY_SET_ID),
        },
        "per_set": per_set,
        "safety_assertions": {
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Vivid Voltage controlled persistence preflight (read-only)")
...
def main() -> int:
    ...
    try:
        payload = run_vivid_voltage_controlled_persistence_preflight(
