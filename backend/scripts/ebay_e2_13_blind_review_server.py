"""Local, blinded, append-only human review server for the EBAY_E2_13 fresh
blind cohort (backend/artifacts/index_fair_value/ebay_e2_13_fresh_blind_queue.csv),
captured under frozen CAPTURE-ALLOCATION-v2 (8 rows/available card), to be
used for a FUTURE COMBINED-IDENTITY-v2 certification attempt (D3-v5 +
IMAGE-v2 + policy v2 -- E2.9B remains consumed and is never re-certified).

This module NEVER imports ebay_d3_matcher_v5, IMAGE-v2, or
ebay_combined_identity_policy_v1/v2, and never computes or displays any
matcher/image/policy-derived value -- a direct architecture reuse of the
corrected V5/E2.9B review UX, using entirely new, E2.13-specific artifact
names so no V4/V5/E2.9B evidence is ever touched. This session starts
ACTIVE at row 0 -- there is no predecessor session to invalidate; the
invalidate/reset machinery is retained ONLY as a capability for a future
defect, not because one occurred here.
"""
...
QUEUE_PATH = OUT / "ebay_e2_13_fresh_blind_queue.csv"
MANIFEST_PATH = OUT / "ebay_e2_13_fresh_blind_manifest.json"
HISTORY_PATH = OUT / "ebay_e2_13_fresh_blind_review_history.jsonl"
CORRECTION_HISTORY_PATH = OUT / "ebay_e2_13_fresh_blind_correction_history.jsonl"

# --------------------------------------------------------------------------
# Review sessions (E2.3B) -- the FIRST human review session
# (ebay_e2_13_fresh_blind_review_history.jsonl, no explicit session identity)
# was invalidated for a UI navigation/rendering defect: Previous/Undo could
# change the reviewed counters while the on-screen image failed to update to
# match, so the reviewer could not guarantee a saved label corresponded to
# the displayed listing. That history is PRESERVED for provenance and is
# permanently excluded from effective labels and from certification. All
# review after the reset happens in a NEW, separately-identified session
# with its own append-only history file.
# --------------------------------------------------------------------------

LEGACY_SESSION_ID = "e2_13_session_1"
SESSION_INVALIDATED_STATUS = "INVALIDATED_UI_NAVIGATION_RENDERING_DEFECT"
SESSION_ACTIVE_STATUS = "ACTIVE"
CERTIFICATION_BLOCKED_INVALID_SESSION = "EBAY_E2_13_CERTIFICATION_BLOCKED_INVALID_REVIEW_SESSION"


def session_history_path(session_id: str) -> Path:
    if session_id == LEGACY_SESSION_ID:
        # SESSION 1 -- INVALIDATED, never written again. Looked up as the
        # live module global (not snapshotted) so tests can monkeypatch
        # HISTORY_PATH and so a real OUT-directory move is respected.
        return HISTORY_PATH
    return HISTORY_PATH.parent / f"ebay_e2_13_fresh_blind_review_history_{session_id}.jsonl"


def _history_fingerprint(events: list[dict[str, Any]]) -> str:
    return hashlib.sha256(
        "\n".join(sorted(f"{e.get('event_id')}:{e.get('action')}" for e in events)).encode()
    ).hexdigest()


def get_active_session_id() -> str:
    """Before the first reset has ever run, the active session is the
    original, un-identified history file -- LEGACY_SESSION_ID.
    """
    manifest = _read_manifest()
    return manifest.get("active_review_session_id") or LEGACY_SESSION_ID


def get_session_record(session_id: str) -> dict[str, Any]:
    manifest = _read_manifest()
    return manifest.get("review_sessions", {}).get(session_id, {})


def is_session_invalidated(session_id: str) -> bool:
    return get_session_record(session_id).get("status") == SESSION_INVALIDATED_STATUS


def active_history_path() -> Path:
    return session_history_path(get_active_session_id())


def invalidate_current_session_and_start_new(reviewer_id: str, reason: str, new_session_id: str = "e2_13_session_2") -> dict[str, Any]:
    """Phase A + Phase B of E2.3B: preserve + invalidate the current
    (defective) review session, then create a fresh session over the SAME
    417-row frozen cohort with reviewed=0/417, no inherited labels, and the
    matcher still unseen. Idempotent-refusing: never re-invalidates a
    session that is already invalidated, and never overwrites an existing
    session id.
    """
    manifest = _read_manifest()
    if not manifest:
        raise FreezeRefused("EBAY_D3_V5_RESET_BLOCKED_NO_MANIFEST")
    if is_frozen():
        raise FreezeRefused("EBAY_D3_V5_RESET_BLOCKED_ALREADY_FROZEN", "cannot reset a session after --freeze has run")

    old_session_id = get_active_session_id()
    if is_session_invalidated(old_session_id):
        raise FreezeRefused("EBAY_E2_13_RESET_BLOCKED_ALREADY_INVALIDATED", old_session_id)

    sessions = manifest.setdefault("review_sessions", {})
    if new_session_id in sessions:
        raise FreezeRefused("EBAY_E2_13_RESET_BLOCKED_SESSION_ID_EXISTS", new_session_id)

    old_path = session_history_path(old_session_id)
    old_events = read_history(old_path, actions=frozenset({"label", "undo"}))
    now = datetime.now(timezone.utc).isoformat()

    sessions[old_session_id] = {
        "status": SESSION_INVALIDATED_STATUS,
        "history_path": old_path.name,
        "row_action_count": len(old_events),
        "history_fingerprint": _history_fingerprint(old_events),
        "invalidated_at": now,
        "reason": reason,
    ...
    new_path = session_history_path(new_session_id)
    sessions[new_session_id] = {
        "status": SESSION_ACTIVE_STATUS,
        "history_path": new_path.name,
        "created_at": now,
        "created_by": reviewer_id,
        "predecessor_session_id": old_session_id,
        "matcher_predictions_consulted": False,
    }
    manifest["active_review_session_id"] = new_session_id
    MANIFEST_PATH.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")

    rows = load_queue_rows()
    return {
        "old_session_id": old_session_id,
        "old_session_status": SESSION_INVALIDATED_STATUS,
        "old_history_fingerprint": sessions[old_session_id]["history_fingerprint"],
        "old_row_action_count": sessions[old_session_id]["row_action_count"],
        "new_session_id": new_session_id,
        "new_session_history_path": new_path.name,
        "reviewed_rows": 0,
        "remaining_rows": len(rows),
        "total_rows": len(rows),
        "matcher_fingerprint_unchanged": True,
...
def assert_no_forbidden_columns(rows: list[dict[str, Any]]) -> None:
    ...
    if present:
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_MATCHER_COLUMNS_PRESENT", str(sorted(present)))
...
def _session_id_for_history_path(path: Path) -> Optional[str]:
    for session_id, record in _read_manifest().get("review_sessions", {}).items():
        if record.get("history_path") == path.name:
            return session_id
...
def append_event(event: Mapping[str, Any], path: Optional[Path] = None) -> dict[str, Any]:
    """Append to the first-pass review history. Refused once that pass is
    frozen, or once the TARGET session (the active session by default, or
    whichever session owns an explicitly-given path) has been invalidated
    (E2.3B) -- an invalidated session must never receive new events again;
    the only path forward is --reset-session.
    """
    target_path = path or active_history_path()
    owning_session = _session_id_for_history_path(target_path) if path is not None else get_active_session_id()
    if owning_session and is_session_invalidated(owning_session):
        raise ReviewFrozen(f"review session {owning_session} is invalidated; run --reset-session first")
    if is_frozen():
        raise ReviewFrozen("first-pass labels are frozen; no further review-history writes are permitted")
    return _append(event, target_path)
...
# deletes the first-pass ebay_e2_13_fresh_blind_review_history.jsonl.
# --------------------------------------------------------------------------
...
def summary(reviewer_id: str) -> dict[str, Any]:
    ...
    effective = reconstruct_effective_labels(read_history(active_history_path()))
    ...
    result = {
        ...
        "frozen": is_frozen(),
        "finally_frozen": is_finally_frozen(),
        "active_review_session_id": active_session,
        "active_review_session_invalidated": is_session_invalidated(active_session),
        "review_sessions": _read_manifest().get("review_sessions", {}),
    }
    if is_frozen():
...
def freeze_initial(reviewer_id: str) -> dict[str, Any]:
    ...
    if is_frozen():
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_ALREADY_FROZEN")

    active_session = get_active_session_id()
    if is_session_invalidated(active_session):
        raise FreezeRefused(CERTIFICATION_BLOCKED_INVALID_SESSION, active_session)

    rows = load_queue_rows()
    if len(rows) != EXPECTED_ROW_COUNT:
        raise FreezeRefused("EBAY_E2_13_REVIEW_BLOCKED_ROW_COUNT_MISMATCH", f"expected {EXPECTED_ROW_COUNT}, found {len(rows)}")
    assert_no_forbidden_columns(rows)

    if not MANIFEST_PATH.exists():
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_MANIFEST_MISSING")
    ...
    if recorded_fingerprint != recomputed_fingerprint:
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_COHORT_FINGERPRINT_MISMATCH",
    ...
    if incomplete:
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_INCOMPLETE_LABELS", f"{len(incomplete)} rows incomplete, e.g. {incomplete[:5]}")
    ...
    manifest["initial_human_freeze"] = {
        ...
        "review_session_id": active_session,
    }
    MANIFEST_PATH.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    return {
        ...
        "reviewer_protocol": REVIEWER_PROTOCOL, "reviewer_id": reviewer_id, "review_session_id": active_session,
    }
...
def freeze_final(reviewer_id: str) -> dict[str, Any]:
    ...
    """
    if not is_frozen():
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_NO_INITIAL_FREEZE")
    if is_finally_frozen():
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_ALREADY_FINALLY_FROZEN")

    frozen_session = _read_manifest().get("initial_human_freeze", {}).get("review_session_id")
    if frozen_session and is_session_invalidated(frozen_session):
        raise FreezeRefused(CERTIFICATION_BLOCKED_INVALID_SESSION, frozen_session)
    ...
    if missing:
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_CORRECTION_AUDIT_INCOMPLETE",
...
def run_precondition_check_only() -> dict[str, Any]:
    """Local completeness/fingerprint check -- no v5 certifier has been built
    yet (out of scope for E2.3, which stops before labeling/certification).
    Never classifies a single listing; only hashes/compares fingerprints and
    inspects label completeness, mirroring what a future certifier will do.
    """
    rows = load_queue_rows()
    manifest = _read_manifest()
    recomputed_cohort_fp = cohort_fingerprint(rows)
    labeled = sum(1 for r in rows if str(r.get("exact_match_yes_no_uncertain", "")).strip())
    recomputed_label_fp = _canonical_label_fingerprint(rows) if labeled == len(rows) else None
    return {
        "cohort_row_count": len(rows),
        "cohort_fingerprint_recorded": manifest.get("cohort_fingerprint"),
        "cohort_fingerprint_recomputed": recomputed_cohort_fp,
        "cohort_fingerprint_matches": manifest.get("cohort_fingerprint") == recomputed_cohort_fp,
        "labeled_row_count": labeled,
        "labels_complete": labeled == len(rows),
        "final_label_fingerprint_recorded": manifest.get("final_label_fingerprint"),
        "final_label_fingerprint_recomputed": recomputed_label_fp,
        "final_label_fingerprint_matches": (
            manifest.get("final_label_fingerprint") == recomputed_label_fp
            if manifest.get("final_label_fingerprint") is not None else False
        ),
        "note": "No v5 certifier is built yet; this is a completeness/fingerprint check only.",
    }
...
def page(row: Mapping[str, Any], reviewed: int, total: int, position: int = 1, message: str = "") -> str:
    ...
    return f"""<!doctype html><meta charset="utf-8"><title>D3-v5 Fresh Blind Review</title>
...
#reasonBar{{display:none;margin-top:10px}}details{{margin-top:14px}}
...
<h1>D3-v5 fresh blind review &mdash; SINGLE_REVIEWER_BLIND</h1>
<div id="rowHeader">
POSITION: {position} / {total}<br>
REVIEWED: {reviewed} / {total}<br><br>
BENCHMARK ROW:<br><b>{e(row_id)}</b><br><br>
LISTING ITEM ID:<br>{e(str(row.get('listing_item_id', '')))}<br><br>
TARGET:<br>{e(target_line)}<br><br>
...
def correction_page(row: Mapping[str, Any], reviewed: int, total: int, current: Optional[dict[str, Any]] = None) -> str:
    ...
    return f"""<!doctype html><meta charset="utf-8"><title>D3-v5 YES-Label Correction Audit</title>
<style>body{{font:16px system-ui;max-width:1000px;margin:30px auto}}img{{max-height:360px;max-width:480px}}
...
<h1>D3-v5 YES-label correction audit &mdash; SINGLE_REVIEWER_BLIND</h1>
...
def review_existing_page(row: Mapping[str, Any], current_fields: Mapping[str, str], position: int, total: int) -> str:
    ...
    for field, value in current_fields.items():
        if field != "exact_match_yes_no_uncertain" and value and value not in ("SINGLE_CARD", "RAW", "CARD"):
            ...
            break
    return f"""<!doctype html><meta charset="utf-8"><title>D3-v5 Review Existing Labels</title>
<style>body{{font:16px system-ui;max-width:1000px;margin:30px auto}}img{{max-height:360px;max-width:480px}}
button{{padding:10px 16px;margin:4px 6px 4px 0;font-size:16px}}.cols{{display:flex;gap:30px}}
.primary{{padding:18px 28px;font-size:20px}}#keepBtn3{{background:#d7f5d0}}#reasonBar3{{display:none;margin-top:10px}}</style>
<h1>D3-v5 review-existing cleanup &mdash; SINGLE_REVIEWER_BLIND</h1>
<p>{position} / {total}</p>
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="E2.3B: invalidate the current (defective) review session for provenance and start a fresh session at 0/417.",
    )
    parser.add_argument(
        "--reason", default=(
            "UI navigation/rendering defect: clicking Previous/Undo changed the reviewed counters "
            "without reliably updating the rendered listing image, so the reviewer could not guarantee "
            "the saved label corresponded to the displayed listing. Reviewer withdrew confidence in the session."
        ),
        help="Reason recorded on the invalidated session (used only with --reset-session).",
    )
    return parser
...
def _run_primary_review(reviewer_id: str, port: int) -> None:
    ...
    if is_session_invalidated(active_session):
        raise SystemExit(
            f"Active review session {active_session} is INVALIDATED "
            f"({SESSION_INVALIDATED_STATUS}). Run --reset-session to start a fresh session."
        )
    history_path = session_history_path(active_session)
    ...
    def compute_effective() -> dict[str, dict[str, Any]]:
        return reconstruct_effective_labels(read_history(history_path))

    initial_effective = compute_effective()
    initial_index = next_unreviewed_index(rows, initial_effective, -1)
    # POSITION and REVIEWED are independent concepts: `cursor["index"]` is a
    # pure navigation position, never mutated by anything except an explicit
    # Previous/Next/row-jump/label-advance/undo-jump. It is NEVER derived by
    # re-scanning for "the next unreviewed row" on every GET -- that re-scan
    # was the root cause of the reported defect (a GET after Undo could land
    # on a DIFFERENT row than the one whose counters had just changed,
    # because the scan direction/start point did not track the specific row
    # that was undone).
            ...
            with lock:
                try:
                    if self.path == "/undo":
                        ...
                        # whatever the page has already advanced to. After
                        # undoing, navigate EXPLICITLY to the row that was
                        # undone (position tracks that row's index directly,
                        # never a re-scan), and surface a visible confirmation.
                        event = build_undo_last_label_event(read_history(history_path), reviewer_id)
                        if event:
                            append_event(event, history_path)
                            undone_row_id = event["benchmark_row_id"]
                        ...
                        # Previous/Next change POSITION ONLY. They never
                        # append to history and never change REVIEWED.
                        ...
                        displayed_row_id = rows[current_index]["benchmark_row_id"]
                        # Reject a label whose row_id does not match the row
                        # actually displayed to the reviewer right now.
                        ...
                        event = build_label_event(
                            row_id, reviewer_id, data.get("primary", ""),
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.reset_session:
        result = invalidate_current_session_and_start_new(args.reviewer, args.reason)
