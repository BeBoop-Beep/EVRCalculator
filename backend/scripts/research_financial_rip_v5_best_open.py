...
from backend.calculations.evr.best_open_price import (
    BEST_OPEN_PRICE_V2_METHOD_VERSION, ExactBestOpenPriceSearch, PreparedCanonicalCandidate,
)
...
class PreparedV5Candidate(PreparedCanonicalCandidate):
    ...
    def score_candidate(self, price_cents: int) -> dict[str, Any]:
        ...
        control = super().score_candidate(price_cents)
        capital = float(control["actualCommittedCapital"])
        v5 = score_financial_rip_v5_candidate(
        ...
        v4 = self.distribution.score(capital, spec=None)  # V3 control for unchanged raw disclosure
        c = v5["components"]
        record = {**control,
    ...
    def compare(self, score_record: Mapping[str, Any], benchmark: Mapping[str, Any], *, authority: str) -> bool:
        if authority == FINANCIAL_V5:
            return financial_v5_key(score_record) < financial_v5_key(benchmark)
        if authority == OVERALL_V5:
            return overall_v5_key(score_record) < overall_v5_key(benchmark)
...
def run(product_limit: int = 0, start_rank: int = 1):
    client = get_client()
    ...
    for i, source in enumerate(ordered, 1):
        ...
        axis_results = {}
        for axis in (FINANCIAL_V5, OVERALL_V5):
            search = ExactBestOpenPriceSearch(
                ...
                current_quantity=int(source["quantity"]), current_rank=ranks[axis],
                benchmark=benchmarks[axis], prepare_quantity=factory,
                ...
                comparison_authority=axis,
            )
            axis_results[axis] = search.search()
            search.clear()
...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    run(args.product_limit, args.start_rank)
