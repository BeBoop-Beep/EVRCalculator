"""Local, blinded, append-only human review server for the EBAY_E2_9B fresh
blind cohort (backend/artifacts/index_fair_value/ebay_e2_9b_fresh_blind_queue.csv),
certifying COMBINED-IDENTITY-v2 (D3-v5 + IMAGE-v2 + policy v2).
...
corrected V5 review UX (E2.3B: reliable global undo, Previous/Next
navigation, append-only relabeling, --review-existing cleanup, the
canonical shared label fingerprint), using entirely new artifact names so
no V4/V5 evidence is ever touched. This session starts ACTIVE at row 0 --
there is no predecessor session to invalidate; the invalidate/reset
machinery is retained ONLY as a capability for a future defect, not because
one occurred here.
"""
...
QUEUE_PATH = OUT / "ebay_e2_9b_fresh_blind_queue.csv"
MANIFEST_PATH = OUT / "ebay_e2_9b_fresh_blind_manifest.json"
HISTORY_PATH = OUT / "ebay_e2_9b_fresh_blind_review_history.jsonl"
CORRECTION_HISTORY_PATH = OUT / "ebay_e2_9b_fresh_blind_correction_history.jsonl"

# --------------------------------------------------------------------------
# Review sessions (E2.3B) -- the FIRST human review session
# (ebay_e2_9b_fresh_blind_review_history.jsonl, no explicit session identity)
...
LEGACY_SESSION_ID = "e2_9b_session_1"
SESSION_INVALIDATED_STATUS = "INVALIDATED_UI_NAVIGATION_RENDERING_DEFECT"
SESSION_ACTIVE_STATUS = "ACTIVE"
CERTIFICATION_BLOCKED_INVALID_SESSION = "EBAY_E2_9B_CERTIFICATION_BLOCKED_INVALID_REVIEW_SESSION"
...
def session_history_path(session_id: str) -> Path:
    if session_id == LEGACY_SESSION_ID:
        ...
        return HISTORY_PATH
    return HISTORY_PATH.parent / f"ebay_e2_9b_fresh_blind_review_history_{session_id}.jsonl"
...
def invalidate_current_session_and_start_new(reviewer_id: str, reason: str, new_session_id: str = "e2_9b_session_2") -> dict[str, Any]:
    ...
    if is_session_invalidated(old_session_id):
        raise FreezeRefused("EBAY_E2_9B_RESET_BLOCKED_ALREADY_INVALIDATED", old_session_id)

    sessions = manifest.setdefault("review_sessions", {})
    if new_session_id in sessions:
        raise FreezeRefused("EBAY_E2_9B_RESET_BLOCKED_SESSION_ID_EXISTS", new_session_id)
...
# deletes the first-pass ebay_e2_9b_fresh_blind_review_history.jsonl.
# --------------------------------------------------------------------------
...
def freeze_initial(reviewer_id: str) -> dict[str, Any]:
    ...
    if len(rows) != EXPECTED_ROW_COUNT:
        raise FreezeRefused("EBAY_E2_9B_REVIEW_BLOCKED_ROW_COUNT_MISMATCH", f"expected {EXPECTED_ROW_COUNT}, found {len(rows)}")
