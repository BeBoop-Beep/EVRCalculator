...
from backend.calculations.evr.financial_rip_v4_config import FINANCIAL_RIP_V4_COMPONENT_ORDER
...
METHOD_CURRENT = "CURRENT_UNIT_V4"
METHOD_EQUAL = "EQUAL_SPEND_V4"
METHOD_RTP = "PURE_RTP"
...
def load_authoritative_products(client: Any, authority_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    products = [row for row in records if str(row.get("calculation_run_id")) in allowed
                and row.get("financial_rip_v4_status") == "ready"
                and row.get("financial_rip_v4_score") is not None
...
def score_values(values: np.ndarray, committed: float) -> dict[str, Any]:
    # Mirrors production exactly (sealed_product_rip_service.py): V4 is a
    # PROJECTION from the V3 payload, never an independent build.
    v3_payload = build_financial_rip_v3(values, committed)
...
def strict_return_dominator(a: Mapping[str, Any], b: Mapping[str, Any]) -> Optional[str]:
    ...
    A downside metric missing on either side (observed under V4 projection at
    some matched-capital quantities) makes that metric unusable for the
    comparison rather than a crash — mirrors the null-guard already present in
    ``multi_metric_dominator``.
    """
    downside = [m for m in ("medianRetention", "chanceToRecoverCapital", "lossResilience")
...
class StrategyEngine:
    ...
    def build_set(self, run_id: str, set_products: Sequence[Mapping[str, Any]]) -> None:
        ...
        for product in set_products:
            ...
            if abs(float(current["metrics"]["financialRipV4"]) - float(product["financial_rip_v4_score"])) > 0.001:
...
def compare_pair(product_a: Mapping[str, Any], product_b: Mapping[str, Any], sa: Mapping[str, Any], sb: Mapping[str, Any],
    ...
    current_a, current_b = float(product_a["financial_rip_v4_score"]), float(product_b["financial_rip_v4_score"])
    ...
    equal_w = winner(ma["financialRipV4"], mb["financialRipV4"])
...
def summarize_calibration(rows: Sequence[Mapping[str, Any]], score_key: str) -> list[dict[str, Any]]:
    ...
    for (score_band, family), members in sorted(groups.items()):
        medians = {}
        for key in ("rtp", "medianRetention", "chanceToRecoverCapital", "lossResilience",
                    "realisticTailMeanRatio", "jackpotTailMeanRatio"):
            # Not all metrics are populated by the V4 projection at every
            # quantity (e.g. realisticTailMeanRatio carries no V4 weight);
            # skip rather than crash, and record as unavailable.
            present = [float(m[key]) for m in members if m.get(key) is not None]
            medians[key] = float(np.median(present)) if present else None
        out.append({"scoreBand": score_band, "productFamily": family, "n": len(members), **medians})
    return out
...
def calibration_coherence(summary: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for metric in metrics:
        ...
        result[metric] = {
            "eligibleBandCount": sum(len(rows) >= 2 for rows in groups.values()),
            "meanCrossFamilyMedianRange": float(np.mean(ranges)) if ranges else None,
        }
    return result
...
def run_research(client: Any) -> dict[str, Any]:
                ...
                if allocation["quantity"] >= 1:
                    ...
                    feasible.append((p, s)); fixed_strategy_rows.append({"setKey": engine.authority[run_id]["set_canonical_key"], **s,
                        **{k:s["metrics"][k] for k in ("rtp","medianRetention","chanceToRecoverCapital","lossResilience","realisticTailMeanRatio","jackpotTailMeanRatio")},
                        "currentScore": float(p["financial_rip_v4_score"]), "equalScore": s["metrics"]["financialRipV4"]})
    ...
    for p in products:
        ...
        product_summary.append({"setKey":set_key,"sealedProductId":p["sealed_product_id"],"productName":p["product_name"],
            ...
            "guaranteedValueShare":p.get("guaranteed_value_share_of_expected_value"),"currentFinancialRip":float(p["financial_rip_v4_score"]),"rtp":one["rtp"]})
    coverage=[]
    ...
    for p in products:
        ...
        current_cal_rows.append({"productFamily":p["product_family"],"currentScore":float(p["financial_rip_v4_score"]),
    ...
    report={"authority":{"snapshotId":snapshot["id"],"marketDate":snapshot["market_date"],"cohortSize":len(authority_rows),
        ...
        "importantDisagreements":disagreements,"decision":"EQUAL_SPEND_PRODUCT_RIP_V4_PENDING_EVALUATION",
...
def render_markdown(r: Mapping[str, Any]) -> str:
    ...
    for a in r["ascendedHeroes"]["anchoredComparisons"]:
        if len(a["strategies"])>1:
            winner=max(a["strategies"],key=lambda s:s["metrics"]["financialRipV4"])
            lines.append(f"- At `${a['targetBudget']:.2f}` ({a['anchorProductName']}), equal-spend V3 prefers {winner['quantity']} x {winner['productName']} at `${winner['actualCommittedCapital']:.2f}` (RTP {winner['metrics']['rtp']:.3f}, V4 {winner['metrics']['financialRipV4']:.2f}).")
    ...
    for a in r["ascendedHeroes"]["anchoredComparisons"]:
        if len(a["strategies"]) > 1:
            for s in a["strategies"]:
                m=s["metrics"]
                lines.append(f"| {a['anchorProductName']} | {s['productName']} | {s['quantity']} | ${s['actualCommittedCapital']:.2f} | ${s.get('leftoverCapital',0):.2f} | {m['rtp']:.4f} | ${m['expectedValue']:.2f} | {m['financialRipV4']:.2f} | {m['medianRetention']:.4f} | {m['chanceToRecoverCapital']:.4f} | {m['lossResilience']:.2f} | {m['p95ThresholdRatio']:.4f} | {m['p99ThresholdRatio']:.4f} |")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--json",default="logs/equal_spend_product_rip_research_v4.json")
    parser.add_argument("--markdown",default="logs/equal_spend_product_rip_research_v4.md")
