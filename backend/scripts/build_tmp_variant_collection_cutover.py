...
def build():
    ...
    outcome["tests"] = [
        "focused parser/helper/manifest selection: 38 passed",
        "scraper + external identity + cards routing + postcondition selection: 121 passed",
    ]
...
def main():
    result = build(); MANIFEST.write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8"); REPORT.write_text(render(result), encoding="utf-8")
