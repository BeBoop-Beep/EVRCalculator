"""Local, blinded, append-only human review server for the D3-v5 fresh blind
cohort (backend/artifacts/index_fair_value/ebay_d3_v5_fresh_blind_queue.csv).

This module NEVER imports ebay_d3_matcher_v5 (or v4/v3/v2/v1) and never
computes or displays any matcher-derived value. It is a dedicated V5 tool,
using entirely new artifact names so no V4 evidence is ever overwritten.
Architecture is a direct reuse of the corrected V4 review UX (E2.1D/E2.1E):
reliable global undo, Previous/Next navigation, append-only relabeling,
--review-existing cleanup, and the canonical shared label fingerprint.
"""
...
MANIFEST_PATH = OUT / "ebay_d3_v5_fresh_blind_manifest.json"
HISTORY_PATH = OUT / "ebay_d3_v5_fresh_blind_review_history.jsonl"
CORRECTION_HISTORY_PATH = OUT / "ebay_d3_v5_fresh_blind_correction_history.jsonl"

# --------------------------------------------------------------------------
# Review sessions (E2.3B) -- the FIRST human review session
# (ebay_d3_v5_fresh_blind_review_history.jsonl, no explicit session identity)
...
LEGACY_SESSION_ID = "v5_session_1"
...
def session_history_path(session_id: str) -> Path:
    if session_id == LEGACY_SESSION_ID:
        ...
        return HISTORY_PATH
    return HISTORY_PATH.parent / f"ebay_d3_v5_fresh_blind_review_history_{session_id}.jsonl"
...
def invalidate_current_session_and_start_new(reviewer_id: str, reason: str, new_session_id: str = "v5_session_2") -> dict[str, Any]:
    ...
    if is_session_invalidated(old_session_id):
        raise FreezeRefused("EBAY_D3_V5_RESET_BLOCKED_ALREADY_INVALIDATED", old_session_id)

    sessions = manifest.setdefault("review_sessions", {})
    if new_session_id in sessions:
        raise FreezeRefused("EBAY_D3_V5_RESET_BLOCKED_SESSION_ID_EXISTS", new_session_id)
...
# deletes the first-pass ebay_d3_v5_fresh_blind_review_history.jsonl.
# --------------------------------------------------------------------------
...
def freeze_initial(reviewer_id: str) -> dict[str, Any]:
    ...
    if len(rows) != EXPECTED_ROW_COUNT:
        raise FreezeRefused("EBAY_D3_V5_REVIEW_BLOCKED_ROW_COUNT_MISMATCH", f"expected {EXPECTED_ROW_COUNT}, found {len(rows)}")
