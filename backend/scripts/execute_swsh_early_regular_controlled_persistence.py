"""Project 17D.3 early regular SWSH controlled persistence execute.
...
- optional dry-run mode (delegates to early-regular no-write runtime audit)
...
DEFAULT_JSON_PATH = Path("logs/audits/swsh_early_regular_controlled_persistence_execute.json")

TARGET_SET_IDS = ("swsh1", "swsh2", "swsh3")
TARGET_ALLOWLIST = set(TARGET_SET_IDS)

EXECUTE_CONFIRMATION_TOKEN = "swsh-early-regular-controlled-persistence-execute"

EXPECTED_TARGET_UUID_BY_SET_ID = {
    "swsh1": "cbc11b3c-0244-4fca-880f-68ebdd599894",
    "swsh2": "759c09f8-8a1b-4212-be89-66088afa6893",
    "swsh3": "3836457c-77dc-44b0-a72f-779b6dd78884",
}
...
def _build_target_lookup() -> Dict[str, Any]:
    lookup: Dict[str, Any] = {}
...
def _controlled_execute_context(*, pack_count: int, target_lookup: Dict[str, Any]):
    ...
    def _resolve_set_config_override(target_set_identifier: str) -> tuple[Any, str]:
        ...
        raise RuntimeError(
            "Controlled execute scope violation: target is outside early-regular allowlist "
...
def run_swsh_early_regular_controlled_persistence_execute(
    *,
    ...
    from backend.scripts.audit_swsh_early_regular_controlled_runtime_dry_run import (
        run_swsh_early_regular_controlled_runtime_dry_run,
    )
    ...
    if dry_run:
        ...
        payload["meta"].update(
            {
                "script": "execute_swsh_early_regular_controlled_persistence.py",
                "project": "17D.3",
        ...
        for set_id in TARGET_SET_IDS:
            ...
            try:
                run_result = orchestrator.run(
                    ...
                    input_source="db",
                    run_metadata={
                        "trigger": "execute_swsh_early_regular_controlled_persistence",
                        "execute": True,
                        "dry_run": False,
                        "set_id": set_id,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Early regular SWSH controlled persistence execute")
    ...
    parser.add_argument("--seed", type=int, default=17431, help="Random seed")
...
def main() -> int:
    ...
    try:
        payload = run_swsh_early_regular_controlled_persistence_execute(
