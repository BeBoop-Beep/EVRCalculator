...
def ranked_targets(rows: Iterable[Dict[str, Any]]) -> List[Dict[str, str]]:
    selected = []
    for row in rows or []:
        # Overall RIP V10 is the canonical rank authority. This selector only
        # identifies already-ranked targets; it never recreates ranking logic.
        # No V9 fallback: a row ranked only under the superseded V9 model is not
        # a canonical rank for this cutover.
        overall = row.get("overallRipV10") if isinstance(row.get("overallRipV10"), dict) else {}
