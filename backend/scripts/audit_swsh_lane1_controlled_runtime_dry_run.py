"""Project 17C.2 regular SWSH Lane 1 controlled runtime dry-run (no writes).
...
DEFAULT_JSON_PATH = Path("logs/audits/swsh_lane1_controlled_runtime_dry_run.json")
...
@contextmanager
def _lane1_no_write_context(*, harness: WriteSpyHarness, pack_count: int, target_lookup: Dict[str, Any]):
    if int(pack_count) <= 0:
    ...
    def _resolve_set_config_override(target_set_identifier: str) -> tuple[Any, str]:
        ...
        raise RuntimeError(
            "Controlled runtime dry-run scope violation: target is outside Lane 1 allowlist "
...
def run_swsh_lane1_controlled_runtime_dry_run(
    *,
    ...
    with _lane1_no_write_context(harness=harness, pack_count=int(pack_count), target_lookup=target_lookup):
        for set_id in TARGET_SET_IDS:
            ...
            run_result = orchestrator.run(
                ...
                input_source="db",
                run_metadata={
                    "trigger": "audit_swsh_lane1_controlled_runtime_dry_run",
                    "dry_run": True,
                    "no_write": True,
                    "set_id": set_id,
    ...
    payload: Dict[str, Any] = {
        "meta": {
            ...
            "project": "17C.2",
            "script": "audit_swsh_lane1_controlled_runtime_dry_run.py",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Lane 1 regular SWSH controlled runtime dry-run audit (no writes)")
