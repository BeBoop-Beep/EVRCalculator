...
# Preview-only provenance. A row carrying one of these sources is a
# candidate index/root input generated by the canonical builder but not yet
# finalized by the rollout finalizer RPC. Dry-run (preview) evaluation may
# treat these as materialized; a --commit publish must never accept them --
# see public_root_materialization's `allow_candidate` parameter.
PUBLIC_ROOT_CANDIDATE_SOURCES = {
    "standard": frozenset({"canonical_root_set_public_rollout_candidate_v1"}),
    "top10": frozenset({"canonical_root_top10_public_rollout_candidate_v1"}),
}
...
def public_root_materialization(
    ...
    *, allow_candidate: bool = False,
    ...
    ``allow_candidate=False`` (the default, and the ONLY mode a --commit
    publish may use) accepts FINAL provenance only
    (current rollout or explicit historical-backfill sources).
    ``allow_candidate=True`` (dry-run
    / preview only) additionally accepts the matching
    ``..._candidate_v1`` provenance, which lets a preview build run before the
    canonical finalizer has produced FINAL rows. Generic/member-only Price
    Storage rows are never accepted in either mode -- they simply are not in
    either source set.
    """
    day = market_day(day)
    ...
    accepted_sources = {
        scope: (
            PUBLIC_ROOT_SOURCES[scope] | PUBLIC_ROOT_CANDIDATE_SOURCES[scope]
            if allow_candidate else PUBLIC_ROOT_SOURCES[scope]
        )
        for scope in PUBLIC_ROOT_SOURCES
    }
    pairs: set[tuple[str, str]] = set()
    duplicates: set[tuple[str, str]] = set()
    seen: set[tuple[str, str]] = set()
    pair_provenance: dict[tuple[str, str], str] = {}
    for row in rows:
        key = (str(row.get("set_id")), str(row.get("value_scope")))
        if key[0] not in roots or key[1] not in accepted_sources:
            continue
        if key in seen:
            duplicates.add(key)
        seen.add(key)
        source = row.get("source")
        if row.get("snapshot_date") == day and source in accepted_sources[key[1]]:
            pairs.add(key)
            is_final = any(source in PUBLIC_ROOT_SOURCES[scope] for scope in (key[1],))
            pair_provenance[key] = "final" if is_final else "candidate"
    expected = {(root, scope) for root in roots for scope in PUBLIC_ROOT_SOURCES}
    missing = sorted(expected - pairs)
    provenance_values = set(pair_provenance.values())
    if not pairs:
        provenance_state = "none"
    elif provenance_values == {"final"}:
        provenance_state = "final"
    elif provenance_values == {"candidate"}:
        provenance_state = "candidate"
    else:
