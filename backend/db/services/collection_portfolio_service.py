...
def _select_with_fallback(
    ...
    for select_clause in select_candidates:
        try:
            ...
            logger.info(
                "public_collection.query correlation_id=%s trace_label=%s table=%s select=%s rows=%s",
                correlation_id,
                trace_label,
                table,
                select_clause,
            ...
            logger.warning(
                "public_collection.query correlation_id=%s trace_label=%s table=%s select=%s error=%s",
                correlation_id,
                trace_label,
                table,
                select_clause,
...
def get_public_collection_data_by_username(
    ...
    logger.info(
        "public_collection.trace correlation_id=%s requested_username=%s normalized_username=%s first_resolves_public_profile=false lookup_strategy=%s row_found=true resolved_user_id=%s resolved_username=%s cards=%s sealed=%s graded=%s collection_items=%s",
        correlation_id,
        requested_username,
        normalized_username,
        trace.get("lookup_strategy"),
