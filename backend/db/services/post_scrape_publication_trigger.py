...
def _market_explorer_v2_current(client: Any, market_date: str) -> bool:
    """True only when every tracked Explorer Set is projected through the date."""
...
def evaluate_post_scrape_publication_currency(
    ...
    *,
    ...
    """Canonical post-scrape audit plus Explorer V2 jointly define CURRENT."""
    try:
...
def _default_publication_current(market_date: str) -> PublicationCurrencyStatus:
    """Durable currency check for canonical publication plus Explorer V2."""
    from backend.db.clients.supabase_client import create_service_role_client

    return evaluate_post_scrape_publication_currency(
        create_service_role_client(), market_date,
    )
