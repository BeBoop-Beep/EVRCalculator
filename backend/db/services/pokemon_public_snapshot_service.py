...
def get_pokemon_set_simulation_evidence_snapshot_payload(set_id: str) -> Dict[str, Any]:
    ...
    resolved = _to_optional_str(set_id)
    if not resolved:
        raise PokemonSetMarketError(400, "set_id is required", "POKEMON_SET_SIMULATION_EVIDENCE_ID_REQUIRED")
    set_row = None if _looks_like_uuid(resolved) else _resolve_set_row(resolved)
    resolved_set_id = resolved if set_row is None else str(set_row["id"])
    result = (
        service_read_client.table("pokemon_set_page_snapshot_latest")
        .select(_INSIGHTS_SNAPSHOT_COLUMNS).eq("set_id", resolved_set_id).limit(1).execute()
    )
    row = _first_row(result)
