-- Rebuild Pokemon set value history from canonical checklist cards first.
--
-- The public set value chart reads pokemon_set_value_daily_history directly.
-- This refresh function keeps the table idempotent by (set_id, date, scope)
-- and records enough coverage diagnostics to validate missing-day gaps.

BEGIN;

ALTER TABLE public.pokemon_set_value_daily_history
    ADD COLUMN IF NOT EXISTS canonical_card_count INTEGER,
    ADD COLUMN IF NOT EXISTS linked_card_count INTEGER,
    ADD COLUMN IF NOT EXISTS included_card_count INTEGER,
    ADD COLUMN IF NOT EXISTS coverage_pct NUMERIC;

CREATE INDEX IF NOT EXISTS idx_cards_set_api_number_for_value_history
    ON public.cards (set_id, pokemon_tcg_api_id, card_number);

CREATE INDEX IF NOT EXISTS idx_card_variants_card_printing_for_value_history
    ON public.card_variants (card_id, pokemon_tcg_api_id, printing_type, special_type);
...
BEGIN
    ...
    hits_aggregated AS (
        SELECT
            ...
            round(sum(lpc.market_price)::numeric, 2) AS set_value,
            count(DISTINCT lpc.pokemon_canonical_card_id)::integer AS priced_card_count,
            count(DISTINCT csf.pokemon_canonical_card_id) FILTER (WHERE csf.is_hit_eligible)::integer AS total_card_count,
            max(cc.canonical_card_count)::integer AS canonical_card_count,
            coalesce(max(lc.linked_card_count), 0)::integer AS linked_card_count,
            count(DISTINCT lpc.pokemon_canonical_card_id)::integer AS included_card_count,
            round(
                (count(DISTINCT lpc.pokemon_canonical_card_id)::numeric /
                    nullif(count(DISTINCT csf.pokemon_canonical_card_id) FILTER (WHERE csf.is_hit_eligible), 0)) * 100,
                2
            ) AS coverage_pct,
