...
-- when the V11/V12 fields were added in
-- 20260906055315_add_v12_v11_to_rankings_sets_lens_rpc.sql. The frontend
-- (frontend/components/explore/ExploreTableClient.jsx, ChaseAccessibilityCell)
-- reads `target.setRipV1.chaseAccessibility` verbatim, so the Chase
-- Accessibility column has been rendering "Unavailable" for every set since
-- that projection stopped shipping the block. This does NOT recompute Chase
-- Accessibility here - it projects the existing authoritative Set-level shape
-- produced by backend/db/services/chase_accessibility_set_ranking.py
-- (project_chase_accessibility_for_set) and attached to Set RIP rows in
-- backend/db/services/set_rip_service.py.
...
SELECT public.project_rankings_json_keys(p_target, ARRAY[
    ...
    'setRipV1', public.project_rankings_json_keys(p_target->'setRipV1', ARRAY[
        ...
        'chaseAccessibility', public.project_rankings_json_keys(p_target#>'{setRipV1,chaseAccessibility}', ARRAY[
