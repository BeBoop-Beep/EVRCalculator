...
create table if not exists public.pokemon_market_explorer_rarity_coverage_certification_v1 (
  singleton boolean primary key default true check (singleton),
  source_from date not null,
  certified_through date not null,
  accepted_date_count integer not null check (accepted_date_count >= 0),
  certified_at timestamptz not null default clock_timestamp()
);

alter table public.pokemon_market_explorer_rarity_coverage_certification_v1 enable row level security;
revoke all on public.pokemon_market_explorer_rarity_coverage_certification_v1
from public,anon,authenticated;
grant select,insert,update,delete
on public.pokemon_market_explorer_rarity_coverage_certification_v1
to service_role;
