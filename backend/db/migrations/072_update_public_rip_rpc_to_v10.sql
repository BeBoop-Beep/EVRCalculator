-- 072: repoint the authoritative publish RPC at the canonical V10 contract.
--
-- WHY THIS EXISTS
-- ---------------
-- Migration 067 pinned the RPC to the V9 identity: it counts ranked targets by
-- `overallRipV9.rank`, requires a ready/rankable `financialRipV3`, and asserts
-- `publicRipContractV9`. The Financial RIP V4 cutover changes three of the four
-- identity strings:
--
...
--     overall rip     : overall_rip_v9_90_financial_v3_10_collector_appeal_v5
--                    -> overall_rip_v10_90_financial_v4_10_collector_appeal_v5
--     public contract : public_rip_contract_v9 -> public_rip_contract_v10
--
-- Collector Appeal V5 is UNCHANGED, and so is the 90/10 split. Only the
-- FINANCIAL input moved (Realistic Upside redefined as the normalized P95
-- threshold ratio alone) - but the identity strings name their inputs, so they
-- move with it, and this function fails closed against the old ones.
--
-- WITHOUT THIS MIGRATION THE CUTOVER CANNOT PUBLISH AT ALL, which is the
-- correct failure mode: the application would build a V10 payload and the
...
--
-- 062 AND 067 ARE NOT EDITED. This is a forward-only CREATE OR REPLACE, so the
-- historical record stays intact and the rollback is to re-apply 067.
--
-- NO V9/V3 FALLBACK EXISTS INSIDE THIS FUNCTION. A V10 publication that cannot
-- satisfy the V10 contract must fail, not silently degrade to the previous
-- model. Historical V9 rows remain readable exactly as they are: this changes
...
-- ca7_version) already carries the model identity, so a V10 snapshot INSERTs as
-- a separate lineage beside a same-date V9 row rather than mutating it.
--
-- WHAT IS PRESERVED VERBATIM FROM 067
-- -----------------------------------
...
--   * ranked targets must carry a ready, rankable Financial RIP (now V4);
...
-- CA7 stopped being canonical. It carries the same V5 string 067 wrote.
...
DECLARE
    ...
    c_overall_rip_version CONSTANT TEXT := 'overall_rip_v10_90_financial_v4_10_collector_appeal_v5';
    c_public_contract_version CONSTANT TEXT := 'public_rip_contract_v10';
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
               ...
               'v10_rank', target #> '{overallRipV10,rank}'
           )), '[]'::JSONB)
      INTO v_canonical
      FROM jsonb_array_elements(v_payload->'targets') AS target
     WHERE target #> '{overallRipV10,rank}' IS NOT NULL
       AND target #> '{overallRipV10,rank}' <> 'null'::JSONB
       AND target #> '{overallRipV10,score}' IS NOT NULL
       AND target #> '{overallRipV10,score}' <> 'null'::JSONB
       ...
       AND target #>> '{publicRipContractV10,contractVersion}' = c_public_contract_version;

    SELECT count(*),
           count(NULLIF(entry->>'set_id', '')),
           count(DISTINCT NULLIF(entry->>'set_id', '')),
           count(DISTINCT (entry->>'v10_rank')::INTEGER),
           min((entry->>'v10_rank')::INTEGER),
           max((entry->>'v10_rank')::INTEGER)
    ...
    IF v_ranked_targets <> v_expected THEN
        RAISE EXCEPTION
            'canonical Overall RIP V10 ranked target count does not match cohort: expected %, received %',
    ...
    IF v_ranked_target_ids <> v_expected OR v_distinct_ranked_target_ids <> v_expected THEN
        RAISE EXCEPTION 'canonical V10 ranked targets contain missing or duplicate stable set IDs';
    END IF;
    ...
    IF v_distinct_ranks <> v_expected THEN
        RAISE EXCEPTION
            'canonical V10 ranks contain duplicates: % distinct ranks across % ranked targets',
            v_distinct_ranks, v_expected;
    END IF;
    IF v_min_rank IS DISTINCT FROM 1 OR v_max_rank IS DISTINCT FROM v_expected THEN
        RAISE EXCEPTION
            'canonical V10 ranks are not contiguous 1..%: observed min=% max=%',
    ...
    IF v_unsupported > 0 THEN
        RAISE EXCEPTION
            'RIP history rows contain % set(s) that are not canonical V10 ranked targets',
            v_unsupported;
    END IF;
