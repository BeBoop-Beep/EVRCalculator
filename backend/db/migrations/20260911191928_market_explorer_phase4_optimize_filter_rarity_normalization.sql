...
select case btrim(
  replace(
    replace(
      replace(
        translate(lower(coalesce(p_rarity,'')),'_-.','   '),
        '  ',' '
      ),
      '  ',' '
    ),
...
'Phase-3 exact Market Explorer card-rarity filter taxonomy. Phase 4 retains identical IDs/membership while replacing the regex normalizer with a cheaper case/separator fold for broad-query latency.';
...
reindex index public.idx_pokemon_market_explorer_card_filter_rarity_v1;
