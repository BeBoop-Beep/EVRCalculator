-- 078: repoint the authoritative publish RPC at the canonical V12 contract.
--
-- WHY THIS EXISTS
-- ---------------
-- Migration 072 pinned the RPC to the V10 identity: it counts ranked targets by
-- `overallRipV10.rank`, requires a ready/rankable `financialRipV4`, and asserts
-- `publicRipContractV10`. The Overall RIP V12 cutover (Chase Accessibility
-- introduced as a third pillar) changes two of the three identity strings:
--
...
--     public contract    : public_rip_contract_v10 -> public_rip_contract_v11
--
-- Financial RIP V4 is UNCHANGED, and so is Collector Appeal V5. Only the
-- OVERALL blend moved (86% Financial V4 / 4% Chase Accessibility V1 / 10%
-- Collector Appeal V5, replacing the prior 90/10 Financial/Collector-Appeal
-- split) - but the identity strings name the blend, so they move with it, and
-- this function fails closed against the old ones.
--
-- WITHOUT THIS MIGRATION THE CUTOVER CANNOT PUBLISH AT ALL, which is the
-- correct failure mode: the application would build a V12 payload and the
-- database would refuse it as "overall RIP version ... is not the canonical
-- ...". A loud transactional rejection, never a half-published leaderboard.
--
-- 067 AND 072 ARE NOT EDITED. This is a forward-only CREATE OR REPLACE, so the
-- historical record stays intact and the rollback is to re-apply 072.
--
-- NO V10 FALLBACK EXISTS INSIDE THIS FUNCTION. A V12 publication that cannot
-- satisfy the V12 contract must fail, not silently degrade to the previous
-- model. Historical V10 rows remain readable exactly as they are: this changes
-- only what may be WRITTEN from now on, and the snapshot uniqueness key
-- (market_date, cohort_version, overall_rip_version, financial_rip_version,
-- ca7_version) already carries the model identity, so a V12 snapshot INSERTs
-- as a separate lineage beside a same-date V10 row rather than mutating it.
--
-- WHAT IS PRESERVED VERBATIM FROM 072
-- -----------------------------------
-- Everything except the identity strings and the payload key paths:
--   * one atomic transaction, history rows inserted BEFORE `latest` is promoted;
--   * idempotent snapshot conflict behaviour on the version-aware key;
--   * the single materialised `v_canonical` definition of a ranked target;
--   * ranked targets must carry a ready, rankable Financial RIP (still V4);
...
--
-- Chase Accessibility itself is NOT recomputed or validated here: it is
-- already folded into `overallRipV12.score`/`.rank` by the application before
-- publication (see backend/desirability/scoring_config.py), and this RPC only
-- ever reads the resulting Overall RIP rank/score, exactly as it read
-- `overallRipV10` before. No new SQL-level readiness predicate is introduced;
-- the application publication contract already gates readiness.
--
-- A NOTE ON `ca7_version`
-- -----------------------
-- Still the canonical Collector Appeal version column, unchanged since before
-- CA7 stopped being canonical. It carries the same V5 string 072 wrote.
