...
BEGIN
    ...
    WITH completed AS MATERIALIZED (
        ...
        DO UPDATE SET
            status = 'pending',
            attempts = CASE
                WHEN public.price_storage_v2_shadow_queue.source_completed_at IS NULL
                  OR EXCLUDED.source_completed_at > public.price_storage_v2_shadow_queue.source_completed_at
                THEN 0
                ELSE public.price_storage_v2_shadow_queue.attempts
            END,
            last_error = NULL,
            started_at = NULL,
            completed_at = NULL,
        ...
        WHERE public.price_storage_v2_shadow_queue.source_completed_at IS NULL
           OR EXCLUDED.source_completed_at > public.price_storage_v2_shadow_queue.source_completed_at
           OR (
                public.price_storage_v2_shadow_queue.status = 'failed'
                AND public.price_storage_v2_shadow_queue.attempts < 5
           )
        RETURNING 1
    )
...
'Idempotently queues completed scrape jobs for Price Storage V2 projection. Same-source terminal failures keep their exhausted retry budget; a newer scrape source resets attempts to zero.';
