...
BEGIN
    ...
    /*
     * Only the tail from p_start_date can change during an incremental sync.
     * The previous implementation windowed the entire event history for every
     * variant in the set before filtering to the tail. Large modern sets can
     * have tens of thousands of historical event rows, which made this stage
     * hit PostgREST's statement timeout even when the requested tail was one
     * day.
     *
     * Keep exactly one pre-tail baseline row per variant, then union only the
     * requested tail. This preserves the price-change boundary comparison while
     * making work proportional to set cardinality + tail size instead of full
     * history size.
     */
    WITH scoped_metadata AS MATERIALIZED (
        ...
        JOIN LATERAL (
            ...
            WHERE event_row.card_variant_id = metadata.card_variant_id
              ...
              AND event_row.effective_date < p_start_date
        ...
        SELECT
            combined.card_variant_id,
            combined.set_id,
            combined.effective_date,
            combined.market_price,
            lag(combined.market_price) OVER (
                PARTITION BY combined.card_variant_id
                ORDER BY combined.effective_date, combined.id
        ...
        WHERE effective_date >= p_start_date
          AND market_price IS DISTINCT FROM previous_market_price
    ), intervals AS (
        SELECT
            card_variant_id,
            set_id,
    ...
    UPDATE public.pokemon_market_price_intervals_v2_shadow baseline_row
    SET valid_to = first_new.first_valid_from
    FROM first_new
    WHERE baseline_row.card_variant_id = first_new.card_variant_id
      AND baseline_row.set_id = p_set_id
      AND baseline_row.valid_from < p_start_date
      AND baseline_row.valid_to IS NULL;
...
'Idempotently rebuilds the compact V2 Market Explorer interval tail for one set from a supplied market date. Uses one indexed pre-tail baseline row per variant plus only tail events, avoiding full-history window scans on large sets.';
