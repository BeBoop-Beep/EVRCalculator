-- 20260909030230: source-repo mirror of a live production repair applied
-- directly, out-of-band, by a party with DB access (NOT executed by this
-- migration -- see the top-level "DO NOT RUN" note below).
--
-- WHY THIS EXISTS
-- ----------------
-- Repo migration 078_update_public_rip_rpc_to_v12.sql already repoints
-- publish_pokemon_public_rip_leaderboard(jsonb,jsonb,jsonb) at the Overall
-- RIP V12 identity (overall_rip_v12_86_financial_v4_04_chase_accessibility_
-- v1_10_collector_appeal_v5 / public_rip_contract_v11 / overallRipV12.rank
-- and .score / Financial RIP V4 + Collector Appeal V5 / fail-closed
-- cohort+rank+set-parity validation / SECURITY DEFINER / search_path=public
-- / service_role-only). A repo-wide grep for the V12 identity strings and
-- for `financialRipV4` + `overallRipV12` inside backend/db/migrations turns
-- up only 078 -- there is no separate, competing repo implementation of
-- this function to reconcile against.
--
-- The one documented difference between 078's body and the live repair
-- described for this effort is a statement timeout: the live RPC runs with
-- `SET statement_timeout = '60s'`, which 078 does not set. This migration
-- exists purely to carry that one additional guard into the repo's
-- migration history as a forward-only CREATE OR REPLACE, without touching
-- 078 and without re-deriving or altering any of 078's already-reviewed
-- validation logic, identity constants, or payload handling.
--
-- DO NOT RUN THIS AGAINST ANY DATABASE FROM THIS BRANCH. This effort has no
-- production or dev DB credentials in this sandbox and could not execute or
-- independently verify this migration against a live database. It exists so
-- the repo's migration history matches production reality (a statement
-- timeout is now enforced on this RPC), and so a maintainer with real DB
-- access can diff `pg_get_functiondef('public.publish_pokemon_public_rip_
-- leaderboard(jsonb,jsonb,jsonb)')` against this file before trusting it as
-- canonical. It also gives the repo an explicit reviewable statement of what
-- changed and why, matching the mirroring convention already established by
-- 20260908224500_mirror_set_page_generation_lifecycle_rpcs.sql.
--
-- WHAT CHANGES VS 078
-- --------------------
-- Only the addition of `SET statement_timeout = '60s'` to the function
-- definition. Every identity constant, validation branch, INSERT/UPDATE
-- statement, and grant in 078 is preserved character-for-character below.
-- Function bodies in Postgres are matched by signature, not by which
-- migration last wrote them, so this CREATE OR REPLACE fully supersedes 078
-- at apply time -- it is not a second, parallel implementation.
--
-- NO V10 FALLBACK IS INTRODUCED. This migration does not weaken, bypass, or
-- add a fallback path around any of 078's cohort/rank/set-parity fail-closed
-- validation.
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public SET statement_timeout = '60s'
AS $$
DECLARE
    ...
    c_financial_rip_version CONSTANT TEXT := 'financial_rip_v4_outcome_profile_p95_only_25_20_15_25_10_5';
    c_collector_appeal_version CONSTANT TEXT := 'collector_appeal_v5_contextual_roster_h_only_d_baseline_up4_down2';
    c_overall_rip_version CONSTANT TEXT := 'overall_rip_v12_86_financial_v4_04_chase_accessibility_v1_10_collector_appeal_v5';
    c_public_contract_version CONSTANT TEXT := 'public_rip_contract_v11';
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
               ...
               'v12_rank', target #> '{overallRipV12,rank}'
           )), '[]'::JSONB)
      INTO v_canonical
      FROM jsonb_array_elements(v_payload->'targets') AS target
     WHERE target #> '{overallRipV12,rank}' IS NOT NULL
       AND target #> '{overallRipV12,rank}' <> 'null'::JSONB
       AND target #> '{overallRipV12,score}' IS NOT NULL
       AND target #> '{overallRipV12,score}' <> 'null'::JSONB
       AND target #> '{financialRipV4,score}' IS NOT NULL
       AND target #> '{financialRipV4,score}' <> 'null'::JSONB
       AND target #> '{financialRipV4,rank}' IS NOT NULL
       AND target #> '{financialRipV4,rank}' <> 'null'::JSONB
       AND target #>> '{financialRipV4,status}' = 'ready'
       AND target #> '{financialRipV4,rankable}' = 'true'::JSONB
       AND target #>> '{publicRipContractV11,contractVersion}' = c_public_contract_version;

    SELECT count(*),
           count(NULLIF(entry->>'set_id', '')),
           count(DISTINCT NULLIF(entry->>'set_id', '')),
           count(DISTINCT (entry->>'v12_rank')::INTEGER),
           min((entry->>'v12_rank')::INTEGER),
           max((entry->>'v12_rank')::INTEGER)
    ...
    IF v_ranked_targets <> v_expected THEN
        RAISE EXCEPTION
            'canonical Overall RIP V12 ranked target count does not match cohort: expected %, received %',
    ...
    IF v_ranked_target_ids <> v_expected OR v_distinct_ranked_target_ids <> v_expected THEN
        RAISE EXCEPTION 'canonical V12 ranked targets contain missing or duplicate stable set IDs';
    END IF;
    ...
    IF v_distinct_ranks <> v_expected THEN
        RAISE EXCEPTION
            'canonical V12 ranks contain duplicates: % distinct ranks across % ranked targets',
            v_distinct_ranks, v_expected;
    END IF;
    IF v_min_rank IS DISTINCT FROM 1 OR v_max_rank IS DISTINCT FROM v_expected THEN
        RAISE EXCEPTION
            'canonical V12 ranks are not contiguous 1..%: observed min=% max=%',
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'canonical V12 ranked target IDs are missing from the history rows';
    END IF;
    ...
    IF v_unsupported > 0 THEN
        RAISE EXCEPTION
            'RIP history rows contain % set(s) that are not canonical V12 ranked targets',
            v_unsupported;
    END IF;
...
REVOKE ALL ON FUNCTION public.publish_pokemon_public_rip_leaderboard(JSONB, JSONB, JSONB) FROM anon, authenticated;
