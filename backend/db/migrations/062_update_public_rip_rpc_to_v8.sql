-- 062: repoint the authoritative publish RPC at the canonical V8 contract.
--
-- WHY THIS EXISTS
-- ---------------
-- Migration 061 pinned the RPC to the V7 identity: it counted ranked targets by
...
-- correct failure mode: the application would build a V8 payload and the
...
-- rollback is to re-apply 061.
--
-- WHAT IS PRESERVED VERBATIM FROM 061
-- -----------------------------------
...
DECLARE
    ...
    c_collector_appeal_version CONSTANT TEXT := 'collector_appeal_v4_h_only_d_baseline_up4_down2';
    c_overall_rip_version CONSTANT TEXT := 'overall_rip_v8_90_financial_v3_10_collector_appeal_v4';
    c_public_contract_version CONSTANT TEXT := 'public_rip_contract_v8';
...
BEGIN
    ...
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
               ...
               'v8_rank', target #> '{overallRipV8,rank}'
           )), '[]'::JSONB)
      INTO v_canonical
      FROM jsonb_array_elements(v_payload->'targets') AS target
     WHERE target #> '{overallRipV8,rank}' IS NOT NULL
       AND target #> '{overallRipV8,rank}' <> 'null'::JSONB
       AND target #> '{overallRipV8,score}' IS NOT NULL
       AND target #> '{overallRipV8,score}' <> 'null'::JSONB
       ...
       AND target #>> '{publicRipContractV8,contractVersion}' = c_public_contract_version;

    SELECT count(*),
           count(NULLIF(entry->>'set_id', '')),
           count(DISTINCT NULLIF(entry->>'set_id', '')),
           count(DISTINCT (entry->>'v8_rank')::INTEGER),
           min((entry->>'v8_rank')::INTEGER),
           max((entry->>'v8_rank')::INTEGER)
    ...
    IF v_ranked_targets <> v_expected THEN
        RAISE EXCEPTION
            'canonical Overall RIP V8 ranked target count does not match cohort: expected %, received %',
    ...
    IF v_ranked_target_ids <> v_expected OR v_distinct_ranked_target_ids <> v_expected THEN
        RAISE EXCEPTION 'canonical V8 ranked targets contain missing or duplicate stable set IDs';
    END IF;
    ...
    IF v_distinct_ranks <> v_expected THEN
        RAISE EXCEPTION
            'canonical V8 ranks contain duplicates: % distinct ranks across % ranked targets',
            v_distinct_ranks, v_expected;
    END IF;
    IF v_min_rank IS DISTINCT FROM 1 OR v_max_rank IS DISTINCT FROM v_expected THEN
        RAISE EXCEPTION
            'canonical V8 ranks are not contiguous 1..%: observed min=% max=%',
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'canonical V8 ranked target IDs are missing from the history rows';
    END IF;
    ...
    IF v_unsupported > 0 THEN
        RAISE EXCEPTION
            'RIP history rows contain % set(s) that are not canonical V8 ranked targets',
            v_unsupported;
    END IF;
