...
def _run_card_image_sync_update(card_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Persist one idempotent card image update with bounded transient retries."""
    ...
    return run_supabase_with_transient_retry(
        operation,
        operation_name="update_card_image_sync_fields",
    )
...
def update_card_image_sync_fields(card_id: str, update_fields: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        return _run_card_image_sync_update(card_id, payload)
    except APIError as e:
        error_msg = str(e)
        ...
        if duplicate_api_id_conflict:
            ...
            if fallback_payload:
                try:
                    return _run_card_image_sync_update(card_id, fallback_payload)
                except Exception:
                    pass
        raise RuntimeError(f"Failed to update card sync fields: {error_msg}") from e
