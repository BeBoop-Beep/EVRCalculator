-- REVIEW-ONLY PROPOSAL: not installed, not in either executable migration folder.
-- After exact migration-history reconciliation and isolated PostgreSQL tests,
-- generate its migration filename with `supabase migration new` and copy here.
-- This installs separate destinations. It does NOT reroute existing writers,
-- enable a scheduler, rewrite legacy history, or publish public snapshots.
BEGIN;
...
-- service_role may read but cannot enable the release gate or rewrite a committed series.
...
BEGIN
 ...
 -- The identifier comes only from the two literal destinations above.
 EXECUTE format($sql$
 ...
 -- Any incompatible existing generation aborts this entire RPC transaction.
 -- Never resolve a conflicting immutable publication with last-writer-wins.
 EXECUTE format($sql$
