"""Public RIP contract v8 — the canonical projection after the Collector Appeal V4 cutover.

WHAT CHANGED FROM v7
--------------------
v7 published ``overallRip`` as ``0.90 * Financial RIP V3 + 0.10 * Collector
Appeal V3``. v8 publishes the canonical model:

    Overall RIP V8 = 0.90 * Financial RIP V3 + 0.10 * Collector Appeal V4

EXACTLY ONE THING MOVED: the appeal input, from Collector Appeal V3 (the
balanced 0.40D + 0.35H + 0.25P sum) to Collector Appeal V4 (D plus a centred,
asymmetric modifier built from H alone). The SPLIT IS UNCHANGED at 90/10, and
...
v6 builder so the canonical and legacy surfaces cannot disagree about a
financial number.

A new contract key even though the split held, for the same reason Overall RIP
took a new version: the identifier names the INPUTS, not the ratio. A consumer
pinned to ``publicRipContractV7`` keeps receiving a V3-backed projection, which
is what that key has always promised.

v4, v5, v6 AND v7 ARE NOT TOUCHED. All four builders still produce exactly what
they always did, under their own keys, for every existing consumer. v8 is a NEW,
additive key.

TWO FACTORS, NOT THREE — AND THE THIRD IS EXPLAINED, NOT DELETED
----------------------------------------------------------------
Collector Appeal V4 consumes Roster Desirability and Desirable Outcome
Frequency. Dual-Path Depth is NOT an input: the ablation found it changed 3 of
231 pairwise orderings and left Spearman(with P, without P) = 0.9966 for a
universal set-level score.

It is still PUBLISHED, under ``collectorAppeal.diagnostics.dualPathDepth`` and
labelled with its role, rather than dropped. A surface that showed three factors
yesterday needs to be able to say why it shows two today; silence would read as
an outage, and deleting a real measurement to tidy a payload throws away a
metric that remains a candidate input for future Personal Fit work. What it must
never do is appear under ``components``, because that block states what fed the
score.

WHAT THIS CONTRACT DELIBERATELY DOES NOT PUBLISH
------------------------------------------------
Collector Appeal V4's modifier ceiling, its downside damping, its H anchors, the
computed modifier, an executable formula string, its internal thresholds, any
validation statistic, and any candidate-grid metadata. Two points of the curve
determine the line, so publishing the ceiling beside the anchors would be
publishing the formula. What a consumer gets is the score, the status, the two
factor values (each already published on its own terms), high-level factor
...
PUBLIC_RIP_CONTRACT_V8_KEY = "publicRipContractV8"
PUBLIC_RIP_CONTRACT_V8_VERSION = "public_rip_contract_v8"
CONTRACT_VERSION = PUBLIC_RIP_CONTRACT_V8_VERSION
...
def _build_overall_v8(
    overall_v8: Mapping[str, Any], cohort_fingerprint: Optional[str] = None
) -> Dict[str, Any]:
    """The canonical Overall RIP block: 90% Financial RIP V3 + 10% Collector Appeal."""
    components_source = _obj(overall_v8.get("components"))
    absolute = _num(overall_v8.get("score"))
    block: Dict[str, Any] = {
        "score": absolute,
        ...
        "relativeScore": _num(overall_v8.get("relativeScore")),
        "rank": _int(overall_v8.get("rank")),
        "rankedSetCount": _int(overall_v8.get("cohortSize")),
        "tier": overall_v8.get("tier"),
        ...
        "components": {
            ...
            "collectorAppeal": {
                ...
                "weight": OVERALL_RIP_V8_WEIGHTS["collector_appeal"],
                "contribution": _num(
    ...
    if absolute is None:
        block["status"] = overall_v8.get("status") or "unavailable_missing_input"
        block["statusReason"] = overall_v8.get("statusReason")
        block["missingInputs"] = list(overall_v8.get("missingInputs") or [])
        block["fallbackPolicy"] = (
            "Overall RIP V8 requires Financial RIP V3 and Collector Appeal V4. A "
...
def _build_collector_appeal_v4(
    collector: Mapping[str, Any],
    universal: Mapping[str, Any],
    cohort_fingerprint: Optional[str] = None,
) -> Dict[str, Any]:
    """The canonical Collector Appeal block: score, rank, and its D/H factors.
    ...
    No ceiling, damping, anchor, modifier or formula string appears here - see the
    module docstring. The two factor VALUES are published because each is already
    a published metric in its own right; what stays internal is how they combine.

    Dual-Path Depth is published under ``diagnostics``, never under
    ``components``: it is a real measurement and a candidate Personal Fit input,
    ...
    block: Dict[str, Any] = {
        ...
        "cohortFingerprint": cohort_fingerprint,
        "version": appeal.get("version") or COLLECTOR_APPEAL_V4_VERSION,
        "formulaVersion": appeal.get("formulaVersion") or COLLECTOR_APPEAL_V4_FORMULA_VERSION,
        ...
        # NOT `components`. Everything under `components` fed the score;
        # everything here is measured, published and did not. Keeping Dual-Path
        # Depth visible under an honestly-named key is the whole point: deleting
        # it would discard a real measurement that remains a candidate input for
        # future Personal Fit models, while leaving it under `components` would
        # claim it contributed to a Collector Appeal it is not part of.
        "diagnostics": {
            "dualPathDepth": {
                ...
                "note": (
                    "Not an input to Collector Appeal V4. Retained as a diagnostic "
                    "and as a candidate input for future Personal Fit models."
                ),
            },
        },
...
def _build_legacy(target: Mapping[str, Any], v7_contract: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    """
    inherited = _obj(v7_contract.get("legacy"))
    legacy: Dict[str, Any] = {
...
def build_public_rip_contract_v8(target: Mapping[str, Any]) -> Dict[str, Any]:
    """Project one ranked target row into the canonical public v8 contract.

    ``target`` must already carry the canonical ``financialRipV3``,
    ``overallRipV8``, ``openingExperience`` (the Collector Appeal payload),
    ``universalSetDesirability``, and the legacy ``rip`` / ``ripCore`` /
    ``overallRipV5`` / ``overallRipV6`` / ``overallRipV7`` objects. Nothing is
    recomputed.

    The Financial RIP block and the legacy blocks are lifted from the v7/v6/v5
    contract builders, so the canonical and legacy surfaces cannot disagree about
    a financial number.
    """
    v7_contract = build_public_rip_contract_v7(target)
    ...
    financial = dict(v7_contract["financialRip"])
    financial["cohortFingerprint"] = cohort_fingerprint

    return {
        "contractVersion": CONTRACT_VERSION,
        "canonicalFinancialRipVersion": CANONICAL_FINANCIAL_RIP_VERSION,
        "canonicalOverallRipVersion": OVERALL_RIP_V8_VERSION,
        "canonicalCollectorAppealVersion": COLLECTOR_APPEAL_V4_VERSION,
        "overallRip": _build_overall_v8(
            _obj(target.get("overallRipV8")), cohort_fingerprint
        ),
        ...
        "collectorAppeal": _build_collector_appeal_v4(
            collector, _obj(target.get("universalSetDesirability")), cohort_fingerprint
        ),
        "universalSetDesirability": v7_contract["universalSetDesirability"],
        "legacy": _build_legacy(target, v7_contract),
        "metricDistinction": {
        ...
        "audit": v7_contract.get("audit") or {},
    }
