...
def test_canonical_scoring_metadata_is_consistently_v5_v9():
    ...
    assert selection["overallRipWeights"] == OVERALL_RIP_V9_WEIGHTS
    assert selection["overallRipEffectiveWeights"] == OVERALL_RIP_V9_EFFECTIVE_WEIGHTS
    assert "Collector Appeal V5" in selection["note"]
    assert "Overall RIP V9" in selection["note"]
