...
def test_currency_is_stale_when_canonical_passes_but_market_explorer_v2_is_stale(monkeypatch):
    ...
    status = trigger.evaluate_post_scrape_publication_currency(
        object(),
        "2026-09-20",
        audit_runner=lambda client, market_date, phase: _AuditReport(market_date, True),
    )

    assert status == trigger.PublicationCurrencyStatus.STALE


def test_currency_is_current_only_when_canonical_and_market_explorer_v2_are_current(monkeypatch):
    seen = []
    monkeypatch.setattr(
        trigger,
...
def test_currency_fails_closed_unknown_when_v2_authority_check_errors(monkeypatch):
    def broken_v2(client, market_date):
        raise RuntimeError("coverage authority unavailable")

    monkeypatch.setattr(trigger, "_market_explorer_v2_current", broken_v2)
