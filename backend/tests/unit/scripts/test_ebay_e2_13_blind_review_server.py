...
import backend.scripts.ebay_e2_13_blind_review_server as server
...
def env(tmp_path, monkeypatch):
    rows = [make_row(i, card_id=f"c{i % 5}") for i in range(553)]
...
# 1. queue loads exactly N rows
def test_queue_loads_exactly_n_rows(env):
    assert len(server.load_queue_rows()) == 553
...
def test_page_html_never_contains_matcher_field_names(env):
    ...
    row = env["rows"][0]
    rendered = server.page(row, 0, 553)
...
# 12. freeze materializes exactly N labels
def test_freeze_materializes_all_n_labels(env):
    label_all(env)
    result = server.freeze("Donny")
    assert result["rows_materialized"] == 553
...
def test_final_effective_labels_reflect_corrections(env):
    freeze_with_mixed_labels(env, yes_count=3, no_count=550)
    rows = server.load_queue_rows()
...
def test_e21e_review_existing_page_contains_no_matcher_fields(env):
    freeze_with_mixed_labels(env, yes_count=5, no_count=415)
    row = env["rows"][0]
    rendered = server.review_existing_page(row, {f: "" for f in server.LABEL_FIELDS}, 1, 553)
    matcher_specific = {"matcher_version", "matcher_state", "identity_state", "match_status",
                         "confidence_tier", "rejection_reason", "v3_state", "v4_state"}
    for forbidden in matcher_specific:
        assert forbidden not in rendered
...
def test_reset_session_preserves_and_invalidates_old_history(env):
    ...
    assert result["remaining_rows"] == 553
...
def test_new_session_starts_at_zero_with_no_inherited_labels(env):
    server.append_event(server.build_label_event("D4-0000", "Donny", "YES"))
    ...
    assert result_summary["remaining_rows"] == 553
...
def test_page_displays_position_reviewed_and_row_identity_together(env):
    row = env["rows"][3]
    html_out = server.page(row, reviewed=2, total=553, position=4, message="")
    assert "POSITION: 4 / 553" in html_out
    assert "REVIEWED: 2 / 553" in html_out
...
def test_page_image_element_is_keyed_to_benchmark_row_id(env):
    row = env["rows"][7]
    html_out = server.page(row, reviewed=0, total=553, position=8)
...
def test_page_shows_undo_confirmation_message_when_provided(env):
    ...
    html_out = server.page(row, reviewed=0, total=553, position=1, message=msg)
    assert msg in html_out


def test_page_has_previous_and_next_controls_distinct_from_undo(env):
    row = env["rows"][0]
    html_out = server.page(row, reviewed=0, total=553, position=1)
