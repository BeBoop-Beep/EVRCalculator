...
import backend.scripts.ebay_d3_v5_blind_review_server as server
...
def env(tmp_path, monkeypatch):
    rows = [make_row(i, card_id=f"c{i % 5}") for i in range(417)]
...
def test_queue_loads_exactly_420_rows(env):
    assert len(server.load_queue_rows()) == 417
...
def test_freeze_materializes_all_420_labels(env):
    ...
    assert result["rows_materialized"] == 417
...
def test_reset_session_preserves_and_invalidates_old_history(env):
    ...
    assert result["remaining_rows"] == 417
...
def test_new_session_starts_at_zero_of_417_with_no_inherited_labels(env):
    server.append_event(server.build_label_event("D4-0000", "Donny", "YES"))
    ...
    assert result_summary["remaining_rows"] == 417
...
def test_page_displays_position_reviewed_and_row_identity_together(env):
    row = env["rows"][3]
    html_out = server.page(row, reviewed=2, total=417, position=4, message="")
    assert "POSITION: 4 / 417" in html_out
    assert "REVIEWED: 2 / 417" in html_out
...
def test_page_image_element_is_keyed_to_benchmark_row_id(env):
    row = env["rows"][7]
    html_out = server.page(row, reviewed=0, total=417, position=8)
...
def test_page_shows_undo_confirmation_message_when_provided(env):
    ...
    html_out = server.page(row, reviewed=0, total=417, position=1, message=msg)
    assert msg in html_out


def test_page_has_previous_and_next_controls_distinct_from_undo(env):
    row = env["rows"][0]
    html_out = server.page(row, reviewed=0, total=417, position=1)
