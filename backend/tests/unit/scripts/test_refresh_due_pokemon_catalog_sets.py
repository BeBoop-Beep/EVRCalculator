...
def test_dry_run_plans_refresh_without_running_commands(monkeypatch):
    ...
    assert any(
        any(part.endswith("sync_pokemon_collector_identities.py") for part in cmd)
        for cmd in refresh["planned_commands"]
    )
    assert any(
...
def test_code_cards_only_catalog_still_refreshes_sealed_without_card_projection(monkeypatch):
    ...
    assert not any("sync_pokemon_collector_identities.py" in line for line in commands)
...
def test_successful_card_refresh_runs_canonical_and_public_snapshots(monkeypatch):
    ...
    assert any("sync_pokemon_collector_identities.py" in line for line in joined)
    image_index = next(i for i, line in enumerate(joined) if "sync_pokemon_images.py" in line)
    desirability_index = next(i for i, line in enumerate(joined) if "build_pokemon_set_desirability_inputs.py" in line)
    collector_index = next(i for i, line in enumerate(joined) if "sync_pokemon_collector_identities.py" in line)
    assert image_index < desirability_index < collector_index
...
def test_image_sync_failure_is_critical_and_reschedules_short_retry(monkeypatch):
    ...
    assert report["status"] == "failed"
    assert report["critical_failures"] == 1
    assert report["refreshes"][0]["status"] == "pokemon_api_image_sync_failed"
    assert report["refreshes"][0]["retry_scheduled_at"] == "2026-09-22T20:00:00+00:00"
    assert timer_calls == [("job-1", "2026-09-22T20:00:00+00:00")]
    joined = [" ".join(command) for command in calls]
    assert any("sync_pokemon_images.py" in line for line in joined)
    assert not any("build_pokemon_set_desirability_inputs.py" in line for line in joined)


def test_collector_identity_failure_is_critical(monkeypatch):
    timer_calls = _patch_common(monkeypatch)
    calls = []

    def fake_run(command):
        calls.append(command)
        failed = any(part.endswith("sync_pokemon_collector_identities.py") for part in command)
        return {
            "command": command,
            "exit_code": 1 if failed else 0,
            "stdout_tail": "",
            "stderr_tail": "collector identity sync failed" if failed else "",
        }

    monkeypatch.setattr(script, "_run", fake_run)
    monkeypatch.setattr(script, "_retry_at", lambda: "2026-09-22T21:00:00+00:00")

    report = script.run(commit=True, limit=5, max_provider_requests=10)

    assert report["status"] == "failed"
    assert report["critical_failures"] == 1
    assert report["refreshes"][0]["status"] == "collector_identity_sync_failed"
    assert timer_calls == [("job-1", "2026-09-22T21:00:00+00:00")]
    joined = [" ".join(command) for command in calls]
    assert any("build_pokemon_set_desirability_inputs.py" in line for line in joined)
    assert any("sync_pokemon_collector_identities.py" in line for line in joined)
    assert not any("build_pokemon_set_cards_snapshots.py" in line for line in joined)
