...
import backend.scripts.ebay_e2_9b_blind_review_server as server
...
def env(tmp_path, monkeypatch):
    rows = [make_row(i, card_id=f"c{i % 5}") for i in range(414)]
...
# 1. queue loads exactly 414 rows
def test_queue_loads_exactly_414_rows(env):
    assert len(server.load_queue_rows()) == 414
...
def test_page_html_never_contains_matcher_field_names(env):
    ...
    row = env["rows"][0]
    rendered = server.page(row, 0, 414)
...
def test_freeze_refuses_when_incomplete(env):
    ...
    with pytest.raises(server.FreezeRefused) as excinfo:
        server.freeze("Donny")
    assert excinfo.value.reason == "EBAY_D3_V5_REVIEW_BLOCKED_INCOMPLETE_LABELS"
...
def test_freeze_refuses_on_cohort_fingerprint_mismatch(env):
    ...
    with pytest.raises(server.FreezeRefused) as excinfo:
        server.freeze("Donny")
    assert excinfo.value.reason == "EBAY_D3_V5_REVIEW_BLOCKED_COHORT_FINGERPRINT_MISMATCH"
...
def test_freeze_refuses_when_forbidden_column_present(env):
    ...
    with pytest.raises(server.FreezeRefused) as excinfo:
        server.freeze("Donny")
    assert excinfo.value.reason == "EBAY_D3_V5_REVIEW_BLOCKED_MATCHER_COLUMNS_PRESENT"


# 12. freeze materializes exactly 414 labels
def test_freeze_materializes_all_414_labels(env):
    label_all(env)
    result = server.freeze("Donny")
    assert result["rows_materialized"] == 414
...
# 18. local precondition check still refuses before freeze (no v5 certifier exists yet)
def test_local_precondition_check_refuses_before_freeze(env):
    report = server.run_precondition_check_only()
    assert report["labels_complete"] is False
    assert report["final_label_fingerprint_matches"] is False


# 19. local precondition check succeeds (fingerprints agree) after a fixture freeze
def test_local_precondition_check_passes_after_fixture_freeze(env):
    label_all(env)
    server.freeze("Donny")  # initial freeze -- all rows are YES
    ...
    assert report["cohort_fingerprint_matches"] is True
    assert report["final_label_fingerprint_matches"] is True
...
def test_freeze_and_summary_never_call_classify_listing(env, monkeypatch):
    calls = {"n": 0}
    import backend.scripts.ebay_d3_matcher_v5 as v5_module

    original = v5_module.classify_listing

    def spy(*args, **kwargs):
        calls["n"] += 1
        return original(*args, **kwargs)

    monkeypatch.setattr(v5_module, "classify_listing", spy)
    label_all(env)
    server.freeze("Donny")
...
def freeze_with_mixed_labels(env, yes_count=249, no_count=168):
    rows = env["rows"]
...
def test_correction_queue_contains_exactly_current_yes_rows(env):
    freeze_with_mixed_labels(env, yes_count=249, no_count=168)
    rows = server.load_queue_rows()
...
def test_final_effective_labels_reflect_corrections(env):
    freeze_with_mixed_labels(env, yes_count=3, no_count=411)
    rows = server.load_queue_rows()
...
def test_final_freeze_refused_when_correction_audit_incomplete(env):
    ...
    with pytest.raises(server.FreezeRefused) as excinfo:
        server.freeze("Donny")
    assert excinfo.value.reason == "EBAY_D3_V5_REVIEW_BLOCKED_CORRECTION_AUDIT_INCOMPLETE"
...
def test_local_precondition_check_detects_label_fingerprint_mismatch(env):
    freeze_with_mixed_labels(env)
    keep_all_yes(env)
    server.freeze("Donny")
    manifest = json.loads(env["manifest"].read_text(encoding="utf-8"))
...
def test_manifest_without_final_fingerprint_never_passes(env):
    freeze_with_mixed_labels(env)
    keep_all_yes(env)
    server.freeze("Donny")
    manifest = json.loads(env["manifest"].read_text(encoding="utf-8"))
...
def test_review_server_never_touches_matcher_fingerprint(env):
    ...
    before = v5.rule_fingerprint()
    freeze_with_mixed_labels(env)
    keep_all_yes(env)
    server.freeze("Donny")
    after = v5.rule_fingerprint()
    assert before == after
...
def test_correction_tooling_never_calls_certification_metrics(env, monkeypatch):
    ...
    # Even though the v5 server no longer imports this module at all, assert
    # the invariant explicitly: nothing in the v5 workflow can reach it.
...
def test_e21e_review_existing_page_contains_no_matcher_fields(env):
    freeze_with_mixed_labels(env, yes_count=5, no_count=415)
    row = env["rows"][0]
    rendered = server.review_existing_page(row, {f: "" for f in server.LABEL_FIELDS}, 1, 414)
    matcher_specific = {"matcher_version", "matcher_state", "identity_state", "match_status",
                         "confidence_tier", "rejection_reason", "v3_state", "v4_state"}
    for forbidden in matcher_specific:
        assert forbidden not in rendered
...
# --------------------------------------------------------------------------
# E2.3B -- session invalidation + navigation/image-integrity fix
# --------------------------------------------------------------------------


def test_default_active_session_is_legacy_before_any_reset(env):
    assert server.get_active_session_id() == server.LEGACY_SESSION_ID
    assert server.is_session_invalidated(server.LEGACY_SESSION_ID) is False


def test_reset_session_preserves_and_invalidates_old_history(env):
    server.append_event(server.build_label_event("D4-0000", "Donny", "YES"))
    server.append_event(server.build_label_event("D4-0001", "Donny", "NO", no_reason="WRONG_CARD"))
    old_events_before = server.read_history(env["history"])
    assert len(old_events_before) == 2

    result = server.invalidate_current_session_and_start_new("Donny", "reported UI defect")

    assert result["old_session_id"] == server.LEGACY_SESSION_ID
    assert result["old_row_action_count"] == 2
    assert result["reviewed_rows"] == 0
    assert result["remaining_rows"] == 414
    assert result["new_session_id"] != server.LEGACY_SESSION_ID

    old_events_after = server.read_history(env["history"])
    assert old_events_after == old_events_before

    manifest = json.loads(env["manifest"].read_text(encoding="utf-8"))
    assert manifest["review_sessions"][server.LEGACY_SESSION_ID]["status"] == server.SESSION_INVALIDATED_STATUS
    assert manifest["review_sessions"][server.LEGACY_SESSION_ID]["row_action_count"] == 2
    assert manifest["review_sessions"][server.LEGACY_SESSION_ID]["reason"] == "reported UI defect"
    assert manifest["review_sessions"][server.LEGACY_SESSION_ID]["matcher_predictions_consulted"] is False
    assert manifest["review_sessions"][server.LEGACY_SESSION_ID]["labels_eligible_for_certification"] is False
    assert manifest["active_review_session_id"] == result["new_session_id"]
    assert manifest["review_sessions"][result["new_session_id"]]["status"] == server.SESSION_ACTIVE_STATUS


def test_new_session_starts_at_zero_of_414_with_no_inherited_labels(env):
    server.append_event(server.build_label_event("D4-0000", "Donny", "YES"))
    server.invalidate_current_session_and_start_new("Donny", "defect")

    active = server.get_active_session_id()
    assert active != server.LEGACY_SESSION_ID
    new_path = server.session_history_path(active)
    assert server.read_history(new_path) == []
    effective = server.reconstruct_effective_labels(server.read_history(new_path))
    assert effective == {}

    result_summary = server.summary("Donny")
    assert result_summary["reviewed_rows"] == 0
    assert result_summary["remaining_rows"] == 414
    assert result_summary["active_review_session_id"] == active


def test_old_invalidated_session_excluded_from_effective_labels(env):
    server.append_event(server.build_label_event("D4-0000", "Donny", "YES"))
    server.invalidate_current_session_and_start_new("Donny", "defect")
    result_summary = server.summary("Donny")
    assert result_summary["reviewed_rows"] == 0
...
def test_append_event_refused_once_active_session_invalidated(env):
    server.invalidate_current_session_and_start_new("Donny", "defect")
    with pytest.raises(server.ReviewFrozen):
        server.append_event(server.build_label_event("D4-0000", "Donny", "YES"), env["history"])


def test_reset_session_refuses_to_double_invalidate(env):
    server.invalidate_current_session_and_start_new("Donny", "defect")
    with pytest.raises(server.FreezeRefused):
        server.invalidate_current_session_and_start_new("Donny", "defect again")


def test_reset_session_refused_after_freeze(env):
    label_all(env)
    server.freeze("Donny")
    with pytest.raises(server.FreezeRefused):
        server.invalidate_current_session_and_start_new("Donny", "too late")


def test_freeze_refuses_invalidated_active_session(env):
    server.invalidate_current_session_and_start_new("Donny", "defect")
    active = server.get_active_session_id()
    new_path = server.session_history_path(active)
    for row in env["rows"]:
        event = server.build_label_event(row["benchmark_row_id"], "Donny", "YES")
        server.append_event(event, new_path)
    manifest = json.loads(env["manifest"].read_text(encoding="utf-8"))
    manifest["review_sessions"][active]["status"] = server.SESSION_INVALIDATED_STATUS
    env["manifest"].write_text(json.dumps(manifest), encoding="utf-8")
    with pytest.raises(server.FreezeRefused) as exc:
        server.freeze("Donny")
    assert exc.value.reason == server.CERTIFICATION_BLOCKED_INVALID_SESSION


def test_freeze_records_review_session_id(env):
    server.invalidate_current_session_and_start_new("Donny", "defect")
    active = server.get_active_session_id()
    new_path = server.session_history_path(active)
    for row in env["rows"]:
        server.append_event(server.build_label_event(row["benchmark_row_id"], "Donny", "YES"), new_path)
    result = server.freeze("Donny")
    assert result["review_session_id"] == active
    manifest = json.loads(env["manifest"].read_text(encoding="utf-8"))
    assert manifest["initial_human_freeze"]["review_session_id"] == active


def test_matcher_and_cohort_fingerprint_unchanged_by_reset(env):
    manifest_before = json.loads(env["manifest"].read_text(encoding="utf-8"))
    result = server.invalidate_current_session_and_start_new("Donny", "defect")
    manifest_after = json.loads(env["manifest"].read_text(encoding="utf-8"))
    assert manifest_after["cohort_fingerprint"] == manifest_before["cohort_fingerprint"]
    assert result["cohort_fingerprint"] == manifest_before["cohort_fingerprint"]


# --------------------------------------------------------------------------
# Navigation / row-identity / image integrity (Phase C)
# --------------------------------------------------------------------------


def test_page_displays_position_reviewed_and_row_identity_together(env):
    row = env["rows"][3]
    html_out = server.page(row, reviewed=2, total=414, position=4, message="")
    assert "POSITION: 4 / 414" in html_out
    assert "REVIEWED: 2 / 414" in html_out
    assert "BENCHMARK ROW:<br><b>" + row["benchmark_row_id"] + "</b>" in html_out
    assert row["listing_item_id"] in html_out
    assert row["listing_title"] in html_out
    assert "IMAGE ROW: <b>" + row["benchmark_row_id"] + "</b>" in html_out


def test_page_image_element_is_keyed_to_benchmark_row_id(env):
    row = env["rows"][7]
    html_out = server.page(row, reviewed=0, total=414, position=8)
    row_id = row["benchmark_row_id"]
    assert ('id="rowimg-' + row_id + '"') in html_out
    assert ('data-row-id="' + row_id + '"') in html_out
    assert ("_row=" + row_id) in html_out


def test_page_shows_undo_confirmation_message_when_provided(env):
    row = env["rows"][0]
    msg = "UNDID LABEL FOR " + row["benchmark_row_id"]
    html_out = server.page(row, reviewed=0, total=414, position=1, message=msg)
    assert msg in html_out


def test_page_has_previous_and_next_controls_distinct_from_undo(env):
    row = env["rows"][0]
    html_out = server.page(row, reviewed=0, total=414, position=1)
    assert 'id="prevBtn"' in html_out
    assert 'id="nextBtn"' in html_out
    assert 'id="undo"' in html_out
    assert "/previous" in html_out
