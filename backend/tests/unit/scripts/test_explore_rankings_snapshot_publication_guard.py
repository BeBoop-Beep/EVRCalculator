...
def test_daily_rank_movement_compares_v12_against_v12_and_v4_against_v4():
    current = {
        "meta": _canonical_meta("2026-08-01"),
        "targets": [{
            "set_id": "stable-1",
            "overallRipV12": {"rank": 2},
            "financialRipV4": {"rank": 4},
    ...
    previous = {
        ...
        "targets": [{
            "set_id": "stable-1",
            "overallRipV12": {"rank": 5},
            "financialRipV4": {"rank": 3},
            "rip": {"rank": 17},
            "ripCore": {"rank": 19},
        }],
    }
...
def test_a_set_that_did_not_move_publishes_zero_movement():
    ...
    def payload(market_date, v7_rank, v4_rank):
        return {
            ...
            "targets": [{
                "set_id": "sv151",
                "overallRipV12": {"rank": v7_rank},
                "financialRipV4": {"rank": v7_rank},
                "rip": {"rank": v4_rank},
...
def test_movement_is_unavailable_across_a_scoring_model_boundary():
    ...
    current = {
        ...
        "targets": [{"set_id": "s", "overallRipV12": {"rank": 2}, "financialRipV4": {"rank": 2}}],
    }
    legacy_previous = {
        "meta": {
...
def test_two_matched_but_superseded_snapshots_still_publish_no_movement():
    ...
    def legacy(market_date, rank):
        return {
            ...
            "targets": [{"set_id": "s", "rip": {"rank": rank}, "overallRipV12": {"rank": rank}}],
        }
...
def test_a_set_absent_from_the_previous_publication_is_new_not_moved():
    current = {
        "meta": _canonical_meta("2026-08-05"),
        "targets": [{"set_id": "fresh", "overallRipV12": {"rank": 3}, "financialRipV4": {"rank": 3}}],
    }
    previous = {
        "meta": _canonical_meta("2026-08-04"),
        "targets": [{"set_id": "other", "overallRipV12": {"rank": 1}, "financialRipV4": {"rank": 1}}],
    }
...
def test_rank_history_failure_is_unavailable_not_new():
    ...
    current = {
        "meta": _canonical_meta("2026-08-01"),
        "targets": [{"set_id": "stable-1", "overallRipV12": {"rank": 2}, "financialRipV4": {"rank": 2}}],
    }
...
def test_older_snapshot_is_not_substituted_for_previous_day():
    ...
    older = {
        ...
        "targets": [{"set_id": "stable-1", "overallRipV12": {"rank": 8}, "financialRipV4": {"rank": 8}}],
    }
