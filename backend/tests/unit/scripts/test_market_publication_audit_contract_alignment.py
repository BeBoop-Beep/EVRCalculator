...
def _global_target(*, history_point_count=1, windows=None, value=100.0):
    return {
        "setId": "set-1",
...
def test_header_still_fails_an_explicit_future_market_date():
    verdict = runtime._runtime_audit_header_summary(DATE, _page(explicit_market_date="2026-09-17"), [])
...
def test_header_still_fails_when_explicit_date_is_ahead_of_a_dependency():
    dependency = core.SectionVerdict(section=core.SECTION_SEALED_MARKET, observed_date="2026-09-14")
    verdict = runtime._runtime_audit_header_summary(DATE, _page(explicit_market_date=DATE), [dependency])
    assert verdict.applicable is True
    assert verdict.passed is False
...
def test_runtime_query_supports_authority_lte_filter():
    query = runtime._RetryingQuery("pokemon_market_root_authority").select("set_id").eq("enabled", True).lte("activated_market_date", DATE)
...
def test_single_current_set_value_point_does_not_owe_movement_windows():
    verdict = runtime._runtime_audit_global_set_value(DATE, target=_global_target(), canonical_set_value=100.0, in_cohort=True)
    assert verdict.passed is True
    assert verdict.applicable is True
    assert "single-point current Set Value history" in verdict.detail


def test_single_current_set_value_point_still_requires_canonical_value_parity():
    verdict = runtime._runtime_audit_global_set_value(DATE, target=_global_target(value=101.0), canonical_set_value=100.0, in_cohort=True)
    assert verdict.passed is False
    assert "disagrees with the canonical standard set value" in verdict.detail


def test_single_current_set_value_point_still_requires_canonical_source_row():
    verdict = runtime._runtime_audit_global_set_value(DATE, target=_global_target(), canonical_set_value=None, in_cohort=True)
    assert verdict.passed is False
    assert "no canonical standard set-value row exists" in verdict.detail
...
def test_multi_point_set_value_still_fails_when_windows_are_missing():
    ...
    verdict = runtime._runtime_audit_global_set_value(DATE, target=target, canonical_set_value=100.0, in_cohort=True)
    assert verdict.passed is False
...
def test_stale_sealed_snapshot_is_truthful_when_source_never_reached_market_date():
    verdict = runtime._runtime_audit_sealed(DATE, {"market_date": "2026-08-15", "product_count": 1}, True, sealed_source_latest_date=None)
    assert verdict.passed is True
    assert verdict.applicable is True
...
def test_sealed_snapshot_still_fails_when_source_reached_market_date():
    verdict = runtime._runtime_audit_sealed(DATE, {"market_date": "2026-08-15", "product_count": 1}, True, sealed_source_latest_date=DATE)
    assert verdict.passed is False
...
def test_runtime_temporarily_installs_and_restores_all_contract_adapters(monkeypatch):
    ...
    def fake_core_run(client, *, market_date=None, canonical_keys=None, phase=None):
        ...
        captured["global"] = core._audit_global_set_value(DATE, target=_global_target(), canonical_set_value=100.0, in_cohort=True)
        captured["sealed"] = core._audit_sealed(DATE, {"market_date": "2026-08-15", "product_count": 1}, True, sealed_source_latest_date=None)
