CREATE OR REPLACE FUNCTION public.get_pokemon_market_explorer_prepared_constituents_v2(p_market_key text, p_generation_id uuid, p_after_rank integer DEFAULT 0, p_limit integer DEFAULT 100)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE
 SET search_path TO ''
...
begin
  ...
  select * into d from public.pokemon_market_explorer_prepared_serving_directory_v1 where market_key = p_market_key;
  if not found then
