...
def commands(phase,day):
    if phase=='simulations':
        return [str(PY),str(Path(__file__).resolve()),'--action','simulation-body','--market-date',day]
    if phase=='collector':
        return [str(PY),'-m','backend.scripts.operationalize_historical_rip','--as-of-date',day,'--commit']
    if phase=='publication':
        return [str(PY),'-m','backend.scripts.run_daily_opening_publication','--market-date',day,'--gate-wait-attempts','1','--gate-wait-seconds','1','--json']
    if phase=='explorer':
        return [str(PY),'-m','backend.scripts.run_market_explorer_daily_publication','--commit','--market-date',day]
    if phase=='pricing':
...
def start(day,phase):
    ...
    with (STATE/'recovery-start.lock').open('a') as lock:
        ...
        for p in (STATE/'recovery').glob('*/*.json') if (STATE/'recovery').exists() else []:
            r=guard.load(p)
            if r.get('status') in ('running','starting') and running(r.get('pid')):
                raise RuntimeError('existing_recovery_phase_is_running')
        previous=loaded(day,phase)
        if previous.get('status')=='completed':
            emit({'status':'already_completed','receipt':previous});return 0
