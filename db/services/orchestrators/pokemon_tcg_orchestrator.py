...
class PokemonTCGOrchestrator:
    ...
    def ingest(self, data):
        ...
        Args:
            data: Dictionary containing optional sections (set, cards, prices, sealed_products)
                  Must contain a 'set' section to establish context for other sections
        ...
        try:
            ...
            # Step 3: Process Prices (depends on set_id and cards)
            if 'prices' in data and set_id:
                try:
                    prices_result = self.prices_controller.ingest_prices(set_id, data['prices'])
            ...
            # Step 4: Process Sealed Products (depends on set_id)
