...
**These entries only guarantee the OAuth flow.** OAuth's `redirectTo` is always the stable, query-free `https://inthedex.io/auth/callback`, so the literal entries above match it exactly on every origin they're added for. Supabase matches the Site URL's own origin more permissively — any path/query on the Site URL is accepted without a separate allow-list entry — but an *additional* redirect URL (anything other than the Site URL, e.g. `www` or `localhost` when the Site URL is the apex) must match what's actually sent, including the query string, unless the entry itself uses a wildcard.

**Server-generated email links** (signup confirmation, password recovery) are opened directly from an email client, possibly on a different device, so there's no prior request on which to set a cookie. Those links keep the return path in the callback URL's query string — `.../auth/callback?next=...` (`buildAuthCallbackUrlWithNext`) — and that query value varies per request, so it can never be pre-registered as one exact literal entry.

- The signup-confirmation link is always built from the trusted, configured server origin (`getFrontendBaseUrl()` via `lib/runtimeUrls.js`, i.e. `NEXT_PUBLIC_BASE_URL` or the local dev default) — never from the raw request `Host` header, which a client can spoof. In production that origin is the apex, which is also the Site URL, so the query-bearing callback is accepted without any extra allow-list entry.
- The password-reset link is built client-side from `window.location.origin` — whichever origin the visitor is actually on when they request it. If that's the Site URL (apex), it's covered the same way. **If it's `www`, localhost, or a preview origin, the exact `/auth/callback` entry for that origin does NOT cover it** — that entry only matches the query-free OAuth URL, not `...?next=...`. A password-reset request from `www` would fail to match on `www` and fall back to the Site URL's confirmation flow instead.
- If email-link flows genuinely need to work from a non-Site-URL origin, add a narrowly scoped additional redirect URL for that path specifically, e.g. `https://www.inthedex.io/auth/callback?*` — scoped to this one path's query variants, not `https://www.inthedex.io/**`, which would also allow-list unrelated paths on that host. Only add this for an origin that actually needs it.
- If Vercel previews need email-link support, set `NEXT_PUBLIC_BASE_URL` to the preview's own origin in that deployment's environment and add both its exact `/auth/callback` (OAuth) and, if needed, the scoped `?*` variant (email links) to Supabase's redirect allow-list; there is no wildcard/any-host fallback.
