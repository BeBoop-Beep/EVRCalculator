...
// window/limit/snapshot_contract forwarding proof.

test("top-chase route delegates to the shared slim set module proxy", () => {
  assert.ok(
    source.includes('from "@/lib/pokemon/slimSetModuleProxyRoute"'),
    "must import the shared slim set module proxy"
  );
  assert.ok(
    source.includes('proxySlimSetModuleRequest("top-chase", request, context)'),
    'must delegate with the "top-chase" module key'
  );
});

test("top-chase route does not re-implement the proxy locally", () => {
