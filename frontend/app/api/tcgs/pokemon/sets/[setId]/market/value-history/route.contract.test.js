...
test("value-history route defines FAILED_ANALYTICS_CACHE_CONTROL as no-store", () => {
  assert.ok(
...
test("value-history route selects FAILED_ANALYTICS_CACHE_CONTROL when the backend response is not ok", () => {
  assert.ok(
...
test("value-history route does not hardcode public cache-control unconditionally", () => {
...
test("value-history route fetch does not use Next's fetch-level cache (would cache a failed backend response for 300s)", () => {
