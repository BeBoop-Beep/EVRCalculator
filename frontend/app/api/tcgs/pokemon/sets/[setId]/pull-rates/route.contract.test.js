...
test("pull-rates route defines FAILED_ANALYTICS_CACHE_CONTROL as no-store", () => {
  assert.ok(
...
test("pull-rates route selects FAILED_ANALYTICS_CACHE_CONTROL when the backend response is not ok", () => {
  assert.ok(
...
test("pull-rates route does not hardcode public cache-control unconditionally", () => {
...
test("pull-rates route fetch does not use Next's fetch-level cache (would cache a failed backend response for 300s)", () => {
...
test("pull-rates route forwards to the backend pull-rates endpoint", () => {
  assert.ok(source.includes('/tcgs/pokemon/sets/${encodeURIComponent(setId)}/pull-rates'));
});
