...
test("insights route defines FAILED_ANALYTICS_CACHE_CONTROL as no-store", () => {
  assert.ok(
...
test("insights route selects FAILED_ANALYTICS_CACHE_CONTROL when the backend response is not ok", () => {
  assert.ok(
...
test("insights route does not hardcode public cache-control unconditionally", () => {
...
test("insights route fetch does not use Next's fetch-level cache (would cache a failed backend response for 300s)", () => {
...
test("insights route forwards to the backend insights endpoint", () => {
  assert.ok(source.includes('/tcgs/pokemon/sets/${encodeURIComponent(setId)}/insights'));
});
