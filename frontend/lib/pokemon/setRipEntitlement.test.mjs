  ...
  targets: [
    {
      ...
      publicRipContractV10: {
        overallRip: {
          leaderNormalizedScore: 87,
          rank: 2,
          factors: { secret: 1 },
        },
        financialRip: { leaderNormalizedScore: 91, dimensions: [1, 2] },
        collectorAppeal: {
          relativeScore: 66,
          coverage: 0.8,
          diagnostics: { dualPathDepth: { rawValue: 0.4 } },
          topSubjects: [
            {
              subjectName: "Charizard",
              demandShare: 0.31,
              elitePath: {
                cardName: "Mega Charizard",
                imageUrl: "/charizard.png",
                rarity: "SIR",
                modeledProbability: 0.002,
                impliedOdds: 500,
                packsFor50PercentChance: 347,
                packsFor90PercentChance: 1151,
              },
              accessiblePath: {
                cardName: "Charizard ex",
                impliedOdds: 40,
                packsFor50PercentChance: 28,
                packsFor90PercentChance: 91,
              },
            },
          ],
        },
      ...
      ripDecision: {
        ...
        topChase: {
          cardName: "Chase",
          impliedOddsOneInN: 480,
          packsFor50PercentChance: 333,
          packsFor90PercentChance: 1105,
        },
      },
    },
  ],
...
test("Basic set payload keeps facts and strips Index Plus decision intelligence", () => {
  const result = applySetRipEntitlement(payload, {
    id: "basic",
    index_plan: null,
  });
  ...
  assert.deepEqual(target.publicRipContractV10.overallRip, {
    leaderNormalizedScore: 87,
    rank: 2,
  });
  assert.deepEqual(target.publicRipContractV10.financialRip, {
    leaderNormalizedScore: 91,
  });
  assert.equal(target.publicRipContractV10.collectorAppeal.relativeScore, 66);
  assert.equal(
    target.publicRipContractV10.collectorAppeal.topSubjects[0].subjectName,
    "Charizard",
  );
  assert.equal(
    target.publicRipContractV10.collectorAppeal.topSubjects[0].demandShare,
    0.31,
  );
  assert.equal(
    target.publicRipContractV10.collectorAppeal.topSubjects[0].elitePath
      .cardName,
    "Mega Charizard",
  );
  assert.equal(
  ...
  assert.equal(
    target.publicRipContractV10.collectorAppeal.diagnostics,
    undefined,
  );
...
test("Index Plus receives the canonical premium publication unchanged", () => {
  assert.equal(
    applySetRipEntitlement(payload, { id: "plus", index_plan: "plus" }),
    payload,
  );
  assert.equal(
    applySetRipEntitlement(payload, { id: "premium", index_plan: "premium" }),
    payload,
  );
  assert.equal(
    payload.targets[0].publicRipContractV10.collectorAppeal.topSubjects[0]
      .elitePath.packsFor50PercentChance,
    347,
  );
  assert.equal(
    payload.targets[0].publicRipContractV10.collectorAppeal.topSubjects[0]
      .elitePath.packsFor90PercentChance,
    1151,
  );
});
...
test("Basic redaction preserves authoritative V10/V4 set headline scores", () => {
  const source = {
    publicRipContractV10: {
      overallRip: {
        leaderNormalizedScore: 87.4,
        rank: 2,
        tier: "S",
        rankedSetCount: 20,
        components: { productRip: 100 },
      },
      financialRip: {
        leaderNormalizedScore: 92.1,
        rank: 1,
        tier: "S",
        rankedSetCount: 20,
        components: { winFrequency: 99 },
      },
      collectorAppeal: {
        relativeScore: 65.2,
        rank: 8,
        rankedSetCount: 20,
        components: { roster: 70 },
      },
    },
  };
