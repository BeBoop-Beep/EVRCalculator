...
test("Market directory technical failure propagates instead of fabricating empty targets", () => {
  const bodyStart = page.indexOf("export default async function TcgSetRipStatisticsPage");
  const body = page.slice(bodyStart);
  assert.match(body, /activeSetDetailTab === "market"\s*\? await getPokemonSetRouteDirectory/);
...
test("notFound runs only after a successfully obtained authoritative target list", () => {
  const fetchIndex = page.indexOf("const targetsPayload = activeSetDetailTab");
