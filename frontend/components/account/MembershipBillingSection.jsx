...
const PLAN_COPY = { plus: "Deeper statistics, opening economics, market breadth, pull odds, acquisition milestones, prepared intelligence, and single-axis market research.",
...
export default function MembershipBillingSection() {
  ...
  const [status,setStatus]=useState(null); const [loading,setLoading]=useState(true); const [action,setAction]=useState(null); const [error,setError]=useState("");
  const load = useCallback(async () => { setLoading(true); setError(""); try { setStatus(await getBillingStatus()); await refreshUser(); } catch (e) {
    if (e?.status===401) { router.push("/login"); return; } setError("Membership information is temporarily unavailable. Your profile settings are unaffected.");
  } finally { setLoading(false); } },[router,refreshUser]);
  useEffect(()=>{ load(); },[load]);
  const offers=useMemo(()=>selectableOffers(status),[status]);
  async function checkout(offerKey) { if(action) return; setAction(offerKey); setError(""); try { const result=await createCheckoutSession(offerKey); window.location.assign(result.checkoutUrl); }
  ...
  async function portal(){ if(action) return; setAction("portal"); setError(""); try{ const result=await createCustomerPortalSession(); window.location.assign(result.portalUrl); }
  ...
  return <section id="billing" aria-labelledby="billing-heading" className="rounded-2xl border border-[var(--border-subtle)] bg-[var(--surface-panel)] p-6 sm:p-8">
    ...
    {!loading&&status?<div className="mt-6 space-y-6">
      <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-page)] p-4"><p className="text-xs font-semibold uppercase tracking-wider text-[var(--text-secondary)]">Current Plan</p><p className="mt-2 text-lg font-semibold text-[var(--text-primary)]">{planLabel(status.effectivePlan)}</p>
        {status.billingManaged?<><p className="mt-2 text-sm font-medium text-[var(--text-primary)]">{presentation.label}</p>{status.billingPlan&&status.billingPlan!==status.effectivePlan?<p className="mt-1 text-sm text-[var(--text-secondary)]">Stripe subscription: {planLabel(status.billingPlan)}. Current access includes {planLabel(status.effectivePlan)}.</p>:null}{message?<p className="mt-1 text-sm text-[var(--text-secondary)]">{message}</p>:null}<button type="button" onClick={portal} disabled={Boolean(action)} className="mt-4 rounded-lg bg-brand px-4 py-2 text-sm font-semibold text-white hover:bg-brand-dark disabled:cursor-not-allowed disabled:opacity-60">{action==="portal"?"Opening…":"Manage Subscription"}</button></>:status.accessManagedByIndex?<p className="mt-2 text-sm text-[var(--text-secondary)]">Access managed by inDex.</p>:<p className="mt-2 text-sm text-[var(--text-secondary)]">Your account includes public inDex tools and insights.</p>}</div>
      <div className="grid gap-4 md:grid-cols-2">{["plus","premium"].map(plan=><article key={plan} className="rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-page)] p-5"><h3 className="font-semibold text-[var(--text-primary)]">{planLabel(plan)}</h3><p className="mt-2 text-sm leading-relaxed text-[var(--text-secondary)]">{PLAN_COPY[plan]}</p><div className="mt-4 flex flex-wrap gap-2">{offers.filter(key=>offerPlan(key)===plan).map(key=><button key={key} type="button" onClick={()=>checkout(key)} disabled={Boolean(action)} className="rounded-lg border border-[var(--border-subtle)] px-4 py-2 text-sm font-semibold text-[var(--text-primary)] hover:bg-[var(--surface-hover)] disabled:opacity-60">{action===key?"Starting…":`Choose ${offerInterval(key)}`}</button>)}</div>{offers.every(key=>offerPlan(key)!==plan)?<p className="mt-4 text-sm font-medium text-[var(--text-secondary)]">Pricing pending — not yet available.</p>:null}</article>)}</div>
    </div>:null}
