...
import { buildProductParentSetHref, finite } from "./productDetailModel.mjs";
  ...
  return (
    <main className="card-detail-environment index-environment set-detail-glass-scope relative isolate min-h-screen px-4 pb-10 pt-5 text-[var(--text-primary)] sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto max-w-[1400px] space-y-4">
        ...
        {detail.rip.available ? entitled ? <><ProductRipSection detail={detail} /><ProductOpeningProfile rip={detail.rip} /></> : <ProductRipLock /> : <ProductRipSection detail={detail} />}
        <ProductComparisonSection detail={detail} entitled={entitled} />
        <details className="set-glass-surface rounded-2xl border p-4 text-sm text-[var(--text-secondary)]"><summary className="cursor-pointer font-semibold text-[var(--text-primary)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent)]">Methodology & Provenance</summary><ul className="mt-3 grid gap-2 sm:grid-cols-2"><li>Sealed market points are real tracked observations.</li><li>Opening outcomes are modeled, not guaranteed.</li><li>Natural-unit product rank compares only within product family.</li><li>Opening outcomes follow the current model’s independence assumptions.</li><li>Market source: {detail.market.source || "Unavailable"}.</li><li>Recovery uses gross market value; calculation run: {detail.rip.calculationRunId || "Unavailable"}.</li></ul></details>
