...
test("cumulative probability and milestone packs use the canonical independent-pack equations", () => {
  const p = 1 / 21;
  assert.ok(
    Math.abs(cumulativePullProbability(p, 29) - (1 - Math.pow(20 / 21, 29))) <
      1e-12,
  );
  assert.deepEqual(
    probabilityMilestones(p).map(({ packs }) => packs),
    [15, 29, 48, 62],
  );
  assert.equal(packsForMilestone(p, 0.75), 29);
});

test("published backend milestones remain authoritative", () => {
  assert.equal(
    probabilityMilestones(0.1, { packsFor50PercentChance: 8 })[0].packs,
    8,
  );
});
...
test("milestone positions are proportional to real pack counts", () => {
  ...
  assert.ok(
    milestoneXPosition(29, 62) - milestoneXPosition(15, 62) <
      milestoneXPosition(62, 62) - milestoneXPosition(29, 62),
  );
});
...
test("plot pointer coordinates invert the same SVG scale at bounds, milestones, and midpoint", () => {
  ...
  for (const packs of [1063, 2125, 3529, 4591]) {
    assert.ok(
      Math.abs(packsAtPlotX(milestoneXPosition(packs, max), max) - packs) <= 1,
    );
  }
});
...
test("invalid, zero, null, and impossible inputs never emit fake numbers", () => {
  for (const value of [
    ...
    assert.equal(validPullProbability(value), null);
  assert.equal(cumulativePullProbability(0, 10), null);
  assert.equal(packsForMilestone(0.1, 1), null);
...
test("certain pulls handle zero packs and the first eligible pack", () => {
  ...
  assert.equal(packsForMilestone(1, 0.95), 1);
});

test("parent navigation renders canonical route slugs and ignores internal target IDs", () => {
  assert.equal(
    buildCardParentSetHref({
      slug: "ascended-heroes",
      targetId: "ascendedHeroes",
    }),
  ...
  assert.equal(
    buildCardParentSetHref({
      ...
      targetId: "temporalForces",
    }),
    "/TCGs/Pokemon/Sets/temporal-forces",
  );
});
