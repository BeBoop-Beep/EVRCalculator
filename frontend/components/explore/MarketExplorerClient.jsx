...
export default function MarketExplorerClient({
  ...
  const togglePrepared = useCallback((seriesId) => {
    const active = preparedActiveKeys.includes(seriesId);
    if (!canComparePreparedMarkets) {
    ...
    setPreparedActiveKeys((current) => (
      current.includes(seriesId)
        ? current.filter((entry) => entry !== seriesId)
        : [...current, seriesId].slice(0, 25)
    ));
    if (!active) setRequestedDetailSeriesId(seriesId);
    return active ? "removed" : "added";
  }, [canComparePreparedMarkets, clearAllQueries, clearAllSelection, preparedActiveKeys]);
  ...
  const comparePrepared = useCallback((seriesId) => {
    ...
    return togglePrepared(seriesId);
  }, [canComparePreparedMarkets, togglePrepared]);

  const selectPrepared = useCallback((seriesId) => {
    if (canComparePreparedMarkets) return togglePrepared(seriesId);
    setPreparedActiveKeys([seriesId]);
    setRequestedDetailSeriesId(seriesId);
    clearAllSelection();
    clearAllQueries();
    setHiddenSeriesKeys(new Set());
    return "replaced";
  }, [canComparePreparedMarkets, clearAllQueries, clearAllSelection, togglePrepared]);
  ...
  return (
    ...
    >
      ...
      <aside id="explorer-controls" data-market-explorer-sidebar className={`${mobileToolsOpen ? "block" : "hidden"} order-3 min-w-0 space-y-3 desk:order-none desk:col-start-1 desk:row-start-1 desk:block desk:max-h-[calc(100vh-4.75rem)] desk:overflow-y-auto`}>
        ...
        <MarketExplorerBrowse directory={preparedDirectory} directoryStatus={preparedDirectoryStatus} activeKeys={preparedActiveKeys}
          canCompare={canComparePreparedMarkets} onSelect={selectPrepared} onCompare={comparePrepared} onBuild={() => { setBuilderMode("exact"); setBuilderOpen(true); }} />
        <div data-market-explorer-sidebar-section="analyze" className="border-t border-[var(--border-subtle)] px-3 py-3">
          ...
          <MarketExplorerRarityMarkets
            ...
            onSelect={togglePrepared}
            onCompare={comparePrepared}
          />
        ...
        <dialog ref={builderDialogRef} role="dialog" aria-modal="true" aria-hidden={!builderOpen} data-market-explorer-builder-overlay data-market-explorer-zone="build" className={builderOpen ? "fixed inset-0 z-[9999] m-0 flex h-full max-h-none w-full max-w-none items-stretch justify-center border-0 bg-slate-950/80 p-0 backdrop-blur-sm desk:items-center desk:p-6" : "hidden"} aria-labelledby="build-markets-zone-heading">
          <div className={`${styles.explorerZone} ${styles.surfaceQuiet} set-glass-surface flex h-[100dvh] w-full min-w-0 flex-col overflow-hidden bg-[var(--surface-page)] shadow-2xl desk:h-[86vh] desk:max-h-[90vh] desk:w-[min(76rem,calc(100vw-3rem))] desk:rounded-2xl`}>
            <div className="flex flex-none items-center gap-2 border-b border-[var(--border-subtle)] bg-[var(--surface-page)]/70 px-4 py-2 sm:px-6" role="tablist" aria-label="Build Your Market method">
              <button type="button" role="tab" aria-selected={builderMode === "exact"} onClick={() => setBuilderMode("exact")} className={`min-h-9 rounded-md border px-3 text-xs font-semibold ${builderMode === "exact" ? "border-[rgb(45,212,191)] bg-[rgba(45,212,191,.14)] text-[rgb(45,212,191)]" : "border-[var(--border-subtle)] text-[var(--text-secondary)]"}`}>Cards &amp; Products</button>
              <button type="button" role="tab" aria-selected={builderMode === "filters"} onClick={() => setBuilderMode("filters")} className={`min-h-9 rounded-md border px-3 text-xs font-semibold ${builderMode === "filters" ? "border-[rgb(45,212,191)] bg-[rgba(45,212,191,.14)] text-[rgb(45,212,191)]" : "border-[var(--border-subtle)] text-[var(--text-secondary)]"}`}>Custom Filters</button>
              <button type="button" aria-label="Close Build Your Market" onClick={() => setBuilderOpen(false)} className="ml-auto flex min-h-10 min-w-10 items-center justify-center rounded-full border border-[rgba(148,163,184,.45)] bg-white/[.06] text-2xl font-light text-[var(--text-primary)] transition hover:border-white/60 hover:bg-white/[.10]">×</button>
            </div>
            {builderMode === "exact" ? (
      ...
      <section
        ...
        className="order-2 flex min-w-0 flex-col desk:order-none desk:col-start-2 desk:row-start-1"
      ...
      <div data-market-explorer-compare-results className="order-4 mt-1">
        <button
          type="button"
          data-market-explorer-details-drawer-trigger
          aria-expanded={detailOpen}
          onClick={() => setDetailOpen((open) => !open)}
          className="group relative flex min-h-16 w-full items-center justify-center overflow-hidden rounded-xl border border-[rgba(148,163,184,.18)] bg-[rgba(15,23,42,.34)] px-4 text-center backdrop-blur-sm transition hover:border-[rgba(45,212,191,.35)] hover:bg-[rgba(15,23,42,.48)]"
        >
          <span className="absolute inset-0 bg-gradient-to-b from-white/[.015] to-transparent" aria-hidden="true" />
          <span className="relative flex flex-col items-center gap-1">
            <span className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[var(--text-secondary)]">Market details &amp; constituents</span>
            <span className="rounded-full border border-[rgba(45,212,191,.35)] bg-[rgba(45,212,191,.08)] px-4 py-1 text-xs font-semibold text-[rgb(94,234,212)]">
              {detailOpen ? "Hide details" : "View details"}
            </span>
          </span>
        </button>
        {detailOpen ? (
          <div className="mt-2 overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--surface-page)]/55 backdrop-blur-sm">
