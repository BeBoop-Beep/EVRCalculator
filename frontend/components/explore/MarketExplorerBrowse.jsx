...
const SEALED_CATEGORIES = [["sealed", "Sealed Markets"]];
const CATEGORIES = [...CARD_CATEGORIES, ...SEALED_CATEGORIES];
...
export default function MarketExplorerBrowse({ directory = [], directoryStatus = "ready", activeKeys = [], canCompare, onSelect, onCompare, onBuild }) {
  ...
  return <section ref={rootRef} data-market-explorer-browse aria-labelledby="browse-markets-heading" className="relative min-w-0 px-3 pb-3 pt-3">
    ...
    <div role="group" aria-label="Directory asset" data-market-directory-asset-layer className="mb-2 grid grid-cols-3 gap-1 rounded-lg border border-[var(--border-subtle)] p-0.5">
      {ASSET_LAYERS.map(([id, label]) => id === "graded"
        ? <button key={id} type="button" data-market-directory-asset={id} disabled aria-disabled="true" title="Graded markets are not published yet" className="min-h-8 cursor-not-allowed rounded-md px-1 text-[10px] font-semibold text-[var(--text-secondary)] opacity-50">{`${label} · Coming later`}</button>
        : <button key={id} type="button" data-market-directory-asset={id} aria-pressed={assetLayer === id} onClick={() => { setAssetLayer(id); setOpen(null); setSearch(""); setHighlightedIndex(NO_HIGHLIGHT); }} className={`min-h-8 rounded-md px-1 text-xs font-semibold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-400/70 ${assetLayer === id ? "bg-sky-400/15 text-sky-200" : "text-[var(--text-primary)] hover:bg-white/[.05]"}`}>{label}</button>)}
    </div>
    <div className={`grid gap-2 ${assetLayer === "sealed" ? "grid-cols-1" : "grid-cols-2"}`}>
      ...
      {assetLayer === "cards" ? <button type="button" data-market-explorer-build-trigger onClick={onBuild} className="min-h-10 rounded-md border border-[rgb(45,212,191)] bg-[rgba(45,212,191,.16)] px-2 text-xs font-bold text-[rgb(45,212,191)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgb(45,212,191)]"><span aria-hidden="true">＋</span> Build Your Market</button> : null}
    </div>
