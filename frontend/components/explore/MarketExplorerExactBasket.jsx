...
export default function MarketExplorerExactBasket({ currentPlan, editingSeries, onAddQuery, onUpdateQuery, onCancelEdit, onClose, showClose = true }) {
  ...
  return <MarketExplorerExactItemPicker selectedItems={items} onChange={setItems} onClose={onClose} showClose={showClose} onCancelEdit={cancelEdit} onBuild={() => build(false)} onSaveAsNew={editingExact ? () => build(true) : null} buildLabel={editingExact ? "Update Market" : "Build Market"} buildStatus={status} buildMessage={message} executionLocked={!premium} />;
}
