...
export default function SimulationFullReport({ canonical, summary, percentiles, evRepresentativeness = null, openingOutcomeProfile = null, calculationRunId = null }) {
  ...
  return <div data-simulation-full-report className={styles.fullReportDisclosure}>
    ...
    {open ? <div id={panelId} className={styles.fullReportPanel}><p className={styles.fullReportIntro}>Financial RIP is built from six scored dimensions. These statistics include the measurements behind those dimensions and additional simulation diagnostics.</p>
      <section className={styles.outcomeProfileSection} aria-labelledby={`${panelId}-outcomes`}><header><div><h3 id={`${panelId}-outcomes`}>What Happens When You Open a Pack?</h3><p>Modeled chances of landing below, near, or above pack cost.</p></div><InfoPopover text="Gross modeled card market value relative to pack cost. Selling fees, grading and liquidity are not included." /></header>
      {outcome ? <><div className={styles.outcomeHero}><strong>{formatOutcomePercent(outcome.groups[0].probability)}</strong><span>finish under half of pack cost</span><small>About {Math.round(outcome.groups[0].probability * 100)} out of 100 modeled packs</small></div>
      ...
      {evRep ? <section className={styles.evRepSection} aria-labelledby={`${panelId}-ev`}><header><div><h3 id={`${panelId}-ev`}>How Typical Is the Expected Value?</h3><p>EV is the long-run average; a typical pack can land far below it.</p></div><InfoPopover text="Estimated from one million modeled outcomes. Horizons assume independent pack draws and are not opening recommendations." /></header>
      ...
      <p className={styles.evCapture}>A typical modeled pack captures <strong>{formatEvRepPercent(evRep.typicalCapture)}</strong> of long-run EV.</p><p className={styles.evSkew}>The top 1% of outcomes contribute <strong>{formatEvRepPercent(evRep.top1OutcomeEvShare)}</strong> of total EV.</p>
      <div className={styles.evMilestones}><div><span>36 packs</span><strong>{realization36 ? formatEvRepPercent(realization36.probabilityAtLeast80PercentEv) : "Not confirmed"}</strong><small>chance to realize at least 80% of EV</small></div><div><span>80% EV horizon</span><strong>{evRep.realizationHorizon ? formatEvRepPacks(evRep.realizationHorizon.packCount) : "Not confirmed"}</strong></div><div><span>Convergence horizon</span><strong>{evRep.convergenceHorizon ? formatEvRepPacks(evRep.convergenceHorizon.packCount) : "Not confirmed"}</strong></div></div>
