...
export default function EvRepresentativenessSection({ summary, percentiles, evRepresentativeness = null, calculationRunId = null, headingId }) {
  ...
  return <section className={styles.evRepSection} aria-labelledby={headingId}><header><div><h3 id={headingId}>How Closely Does EV Match Real Openings?</h3><p>Expected Value is a long-run average. This shows how closely typical and repeated openings resemble it.</p></div><InfoPopover text="Estimated from one million modeled outcomes. Horizons assume independent pack draws and are not opening recommendations." /></header>
    ...
    {evRep.realizationByPackCount.length ? <details className={styles.evRepRealization}><summary>Explore other pack counts</summary><div role="table" aria-label="EV realization by pack count">{evRep.realizationByPackCount.map((r) => <div role="row" key={r.packCount} className={styles.evRepTableRow}><span role="cell">{r.packCount.toLocaleString("en-US")} packs</span><strong role="cell">{formatEvRepPercent(r.probabilityAtLeast80PercentEv)}</strong></div>)}</div></details> : null}
  </section>;
}
