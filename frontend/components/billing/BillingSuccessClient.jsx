...
export default function BillingSuccessClient(){
  ...
  useEffect(()=>{ let active=true; const controller=new AbortController(); const getStatus=()=>getBillingStatus({signal:controller.signal}); pollBillingConfirmation({getStatus,shouldContinue:()=>active,onStatus:status=>{if(active)setState({phase:"checking",status});}}).then(async result=>{
  ...
  }); return()=>{active=false;controller.abort();};},[refreshUser]);
