...
WITH current_ready AS (
    SELECT c.set_id,c.set_name,s.canonical_key,e.name AS era_name,s.release_date,
           s.logo_image_url,s.symbol_image_url,c.market_scope,c.canonical_market_date,
           c.market_publication_ready,c.current_certification_status,0 AS source_priority
    ...
    WHERE c.market_scope='standard' AND c.market_publication_ready
), rollout AS (
    SELECT r.set_id,r.set_name,r.canonical_key,r.era_name,r.release_date,
           r.logo_image_url,r.symbol_image_url,'standard'::text AS market_scope,
           c.canonical_market_date,true AS market_publication_ready,
           c.current_certification_status,1 AS source_priority
    ...
    JOIN public.pokemon_market_root_set_publication_current_certification_v1 c
      ON c.set_id=r.set_id AND c.market_scope='standard'
...
FROM (SELECT * FROM current_ready UNION ALL SELECT * FROM rollout) u
...
BEGIN
    SELECT max(q.market_date)::date INTO v_latest_approved
    ...
    IF v_market_date IS NULL THEN RAISE EXCEPTION 'No approved Pokemon market date exists'; END IF;
    IF v_market_date IS DISTINCT FROM v_latest_approved THEN
        RAISE EXCEPTION 'Rollout snapshot refresh is current-date only: requested %, latest approved %',v_market_date,v_latest_approved;
    END IF;

    WITH roots AS MATERIALIZED (
        SELECT r.set_id FROM public.pokemon_market_rollout_root_sets_v1 r
        WHERE r.activated_market_date<=v_market_date
          AND (r.release_date IS NULL OR r.release_date<=v_market_date)
    ), canonical AS MATERIALIZED (
        SELECT v.set_id,v.set_value,v.expected_card_count,v.priced_card_count,v.coverage_pct
        FROM public.pokemon_market_root_set_value_latest_v1 v
        JOIN roots r ON r.set_id=v.set_id
        WHERE v.market_scope='standard' AND coalesce(v.coverage_pct,0)>=95
    )
    ...
    SELECT c.set_id,v_market_date,'standard',c.set_value,c.priced_card_count,c.expected_card_count,
           'canonical_root_set_rollout_v1',c.expected_card_count,c.expected_card_count,
           c.priced_card_count,c.coverage_pct,now(),now()
    FROM canonical c
    ON CONFLICT (set_id,snapshot_date,value_scope) DO UPDATE
    SET set_value=EXCLUDED.set_value,priced_card_count=EXCLUDED.priced_card_count,
        total_card_count=EXCLUDED.total_card_count,source=EXCLUDED.source,
        canonical_card_count=EXCLUDED.canonical_card_count,
        linked_card_count=EXCLUDED.linked_card_count,included_card_count=EXCLUDED.included_card_count,
        coverage_pct=EXCLUDED.coverage_pct,updated_at=now();
    ...
    WITH roots AS MATERIALIZED (
        ...
        SELECT t.set_id,sum(t.market_price)::numeric AS set_value,count(*)::integer AS card_count
        ...
        GROUP BY t.set_id HAVING count(*)=10
    )
    INSERT INTO public.pokemon_set_value_daily_history(
        set_id,snapshot_date,value_scope,set_value,priced_card_count,total_card_count,
        source,canonical_card_count,linked_card_count,included_card_count,coverage_pct,
        created_at,updated_at)
    SELECT g.set_id,v_market_date,'top10',g.set_value,g.card_count,10,
           'canonical_root_top10_rollout_v1',10,10,g.card_count,100.00,now(),now()
    FROM grouped g
    ON CONFLICT (set_id,snapshot_date,value_scope) DO UPDATE
    SET set_value=EXCLUDED.set_value,priced_card_count=EXCLUDED.priced_card_count,
        total_card_count=EXCLUDED.total_card_count,source=EXCLUDED.source,
    ...
    WHERE h.snapshot_date=v_market_date AND h.set_id IN (
        SELECT r.set_id FROM public.pokemon_market_rollout_root_sets_v1 r
        WHERE r.activated_market_date<=v_market_date
          AND (r.release_date IS NULL OR r.release_date<=v_market_date));
    ...
    INSERT INTO public.pokemon_set_top_chase_card_daily_history(
        set_id,snapshot_date,card_id,card_variant_id,rank,name,rarity,image_url,
        image_small_url,image_large_url,market_price,source,source_date,created_at,updated_at)
    SELECT t.set_id,v_market_date,t.canonical_card_id,t.card_variant_id,t.rank,t.card_name,t.rarity,
           coalesce(pcc.image_small_url,pcc.image_large_url),pcc.image_small_url,pcc.image_large_url,
           t.market_price,t.source,t.captured_at::date,now(),now()
    FROM public.pokemon_market_root_set_top10_latest_v1 t
    ...
    SET card_id=EXCLUDED.card_id,card_variant_id=EXCLUDED.card_variant_id,name=EXCLUDED.name,
        rarity=EXCLUDED.rarity,image_url=EXCLUDED.image_url,image_small_url=EXCLUDED.image_small_url,
        image_large_url=EXCLUDED.image_large_url,market_price=EXCLUDED.market_price,
        source=EXCLUDED.source,source_date=EXCLUDED.source_date,updated_at=now();
    GET DIAGNOSTICS v_top10_rows=ROW_COUNT;

    RETURN jsonb_build_object(
        'marketDate',v_market_date,
        'rolloutRootCount',(SELECT count(*) FROM public.pokemon_market_rollout_root_sets_v1 r
            WHERE r.activated_market_date<=v_market_date AND (r.release_date IS NULL OR r.release_date<=v_market_date)),
...
END;
$function$;
REVOKE ALL ON FUNCTION public.refresh_pokemon_market_rollout_daily_snapshots_v1(date) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.refresh_pokemon_market_rollout_daily_snapshots_v1(date) TO service_role;
