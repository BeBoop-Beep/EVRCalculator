CREATE OR REPLACE FUNCTION public.refresh_pokemon_set_value_daily_history(
    p_set_id uuid DEFAULT NULL::uuid,
...
BEGIN
    -- captured_at is stored as a DATE in card_variant_price_observations.
    -- Do not timezone-convert it here; doing so shifts midnight UTC dates one
    -- day backward for America/Phoenix and writes set value history to the
    -- wrong snapshot_date.
    SELECT id
    ...
    observed_bounds AS (
        SELECT
            cvl.set_id,
            min(o.captured_at::date) AS first_observation_date,
            max(o.captured_at::date) AS latest_observation_date
    ...
    latest_priced_cards AS (
        ...
        JOIN LATERAL (
            ...
            WHERE cvl_inner.set_id = linked_cards.set_id
              ...
              AND o.market_price > 0
              AND o.captured_at IS NOT NULL
              AND o.captured_at::date <= sd.snapshot_date
