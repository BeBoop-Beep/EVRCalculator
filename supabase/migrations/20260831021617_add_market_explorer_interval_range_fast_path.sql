-- Native range-indexed validity expansion plus a dimension-free common path.
BEGIN;

CREATE INDEX IF NOT EXISTS idx_pokemon_variant_market_intervals_validity_gist
 ON public.pokemon_card_variant_market_price_intervals
 USING gist (daterange(valid_from, valid_to, '[)'))
 INCLUDE (observation_id, card_variant_id, canonical_card_id, set_id,
          market_price, rarity);
...
WITH market_dates AS MATERIALIZED (
...
), filtered_panel AS MATERIALIZED (
 SELECT dates.market_date, dates.previous_market_date,
        fact.observation_id, fact.card_variant_id, fact.market_price
 FROM date_context dates
 JOIN public.pokemon_card_variant_market_price_intervals fact
 ...
 SELECT state.market_date, state.observation_id,
        state.card_variant_id, state.market_price,
...
FROM series
JOIN observed_dates dates ON dates.market_date = series.market_date
