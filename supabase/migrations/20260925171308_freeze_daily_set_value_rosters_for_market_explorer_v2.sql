...
-- Market Explorer V2: freeze exact Set Value leaves inside the current-date SQL publisher.
-- Built from the already-materialized publication work tables; no additional price-resolution scan.
...
DECLARE
  ...
  v_frozen_roots integer := 0;
  v_freeze record;
  v_items jsonb;
  v_raw_methodology text;
BEGIN
  SELECT coalesce((
    SELECT h.methodology_version
    FROM public.pokemon_market_index_daily_history h
    WHERE h.tcg='pokemon' AND h.index_key='raw'
    ORDER BY h.market_date DESC,h.updated_at DESC
    LIMIT 1
  ),'chain_linked_common_cohort_v1')
  INTO v_raw_methodology;
  ...
  -- Freeze the exact priced physical leaves that produced each Standard Set
  -- Value in this same transaction. Raw Explorer composition consumes only
  -- these frozen rows; no later current/latest recomputation is permitted.
  FOR v_freeze IN
    SELECT v.set_id,v.set_value,v.priced_card_count
    FROM pg_temp.pokemon_market_rollout_values_work_v1 v
    WHERE coalesce(v.coverage_pct,0) >= 95
      AND v.priced_card_count > 0
    ORDER BY v.set_id
  LOOP
    SELECT jsonb_agg(
      jsonb_build_object(
        'canonicalCardId',p.canonical_card_id,
        'cardVariantId',p.card_variant_id,
        'setId',p.member_set_id,
        'marketPrice',p.market_price,
        'capturedAt',p.captured_at,
        'source',p.source,
        'printingType',p.printing_type,
        'priceSelectionReason',p.price_selection_reason
      )
      ORDER BY p.canonical_card_id
    )
    INTO v_items
    FROM pg_temp.pokemon_market_rollout_prices_work_v1 p
    WHERE p.root_set_id=v_freeze.set_id
    ...
    PERFORM public.replace_pokemon_market_set_value_constituents_v1(
      ...
      v_raw_methodology,
      v_freeze.set_value,
      v_freeze.priced_card_count,
      'canonical_root_set_public_rollout_candidate_frozen_v1',
      coalesce(v_items,'[]'::jsonb)
    );
    v_frozen_roots:=v_frozen_roots+1;
  END LOOP;
  ...
  RETURN jsonb_build_object(
    ...
    'frozenSetValueRosters', v_frozen_roots,
...
BEGIN
  ...
  LOOP
    ...
    PERFORM public.replace_pokemon_market_set_value_constituents_v1(
      ...
      v_freeze.set_value,
      v_freeze.priced_card_count,
      'canonical_root_set_public_rollout_frozen_v1',
      coalesce(v_items,'[]'::jsonb)
    );
    v_frozen_roots:=v_frozen_roots+1;
  END LOOP;
