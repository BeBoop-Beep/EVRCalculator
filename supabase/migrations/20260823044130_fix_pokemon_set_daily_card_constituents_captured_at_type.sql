-- Corrective fix, same function: card_variant_price_observations.captured_at is
-- actually DATE in production (verified via information_schema), not TIMESTAMPTZ
-- as originally declared. This was caught by the function's first live
-- execution, which failed with "structure of query does not match function
-- result type ... column 6" every single call, before any reconciliation could
-- run. Fixing the declared return type to match reality; the query body itself
-- is unchanged, still read-only, still STABLE, no data mutation.
DROP FUNCTION IF EXISTS public.get_pokemon_set_daily_card_constituents(UUID, DATE, DATE);
