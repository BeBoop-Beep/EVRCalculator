...
COMMENT ON COLUMN public.budget_product_ranking_rows.expected_value IS
    'Mean of the real quantity-Q strategy outcome distribution; never reconstructed from single-unit EV.';

ALTER FUNCTION public.publish_budget_product_ranking_snapshot(JSONB, JSONB)
    RENAME TO publish_budget_product_ranking_snapshot_without_strategy_ev;
REVOKE ALL ON FUNCTION public.publish_budget_product_ranking_snapshot_without_strategy_ev(JSONB, JSONB)
...
BEGIN
    ...
    WHERE snapshot_id = v_snapshot_id AND expected_value IS NOT NULL;
    IF v_row_count <> jsonb_array_length(p_rows) THEN
        RAISE EXCEPTION 'persisted strategy expected values do not reconcile with publication rows';
    END IF;
...
REVOKE ALL ON public.budget_product_ranking_snapshots,
    public.budget_product_ranking_rows,
    public.budget_product_ranking_latest
    FROM PUBLIC, anon, authenticated;
