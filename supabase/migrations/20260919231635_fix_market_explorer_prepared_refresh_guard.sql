...
-- Fail-closed handoff used by the maintained-cache prewarm worker. The full
-- refresh above is transactional; if its coherent comparison watermark does
-- not exactly match the caller's accepted market date, this wrapper raises and
-- PostgreSQL rolls the entire prepared-generation replacement back.
create or replace function public.refresh_pokemon_market_explorer_prepared_directory_if_current_v1(
...
begin
  ...
  if v_comparison_asof is distinct from p_required_market_date then
    raise exception
      'Market Explorer prepared generation not current: comparisonAsOf=% required=%',
      coalesce(v_comparison_asof::text, 'null'),
      p_required_market_date::text;
  end if;
...
revoke all on function public.refresh_pokemon_market_explorer_prepared_directory_if_current_v1(date)
  from public, anon, authenticated;
grant execute on function public.refresh_pokemon_market_explorer_prepared_directory_if_current_v1(date)
  to service_role;

comment on function public.refresh_pokemon_market_explorer_prepared_directory_if_current_v1(date) is
'Fail-closed prepared Market Explorer handoff. Rebuilds transactionally and commits only when comparisonAsOf exactly equals the required accepted market date.';
